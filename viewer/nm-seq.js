...
(function () {
  ...
  /* ── 산출물 캡션 표기 규칙(운영자 260819 «결과값에 opa라는 말을 결과에 안나오게 · 예: opa50 은 50으로 · 오버레이(50) 이런 식으로 축약») ──
   * 거처 사유 = 위 순번과 **같은 축**이다: 「이미지 스튜디오 3칸(카드 제작·번역·AI 생성)이 산출물을 화면에 어떤 이름으로 부르는가」.
   *   그 세 칸은 이력 저장소(nomute_thumb_hist)를 공유해서 카드 제작 산출물이 번역 칸 이력에도 그대로 뜬다(실측 260819:
   *   카드 제작만 고쳤더니 번역 칸에는 「헤더·오버레이(OPA50)」이 남아 한 저장소의 같은 항목이 칸마다 다른 이름으로 보였다).
   *   이 모듈은 그 세 칸이 이미 함께 들고 있어(상속 배선 추가 0) 여기 두면 사본이 생기지 않는다.
   * ⚠ 화면 표기 전용 = 저장 라벨·파일명·서버 변형 라벨(api/thumb.js 'OPA{값}')은 무접촉 — 그 셋은 복원·매칭·경로가 읽는 기계 축이다.
   * ⚠ 숫자가 뒤따를 때만 벗긴다 = 「OPA」가 단독으로 쓰인 다른 문구는 안 건드린다. */
  window.nmCap = { text: function (s) {
    return String(s == null ? '' : s)
      .replace(/OPA(?=\d)/g, '')        // 「OPA50」 → 「50」(260819 1차)
      .replace(/배경 합성/g, '배경')     // 「합성」은 이 자리에서 늘 같은 뜻이라 없어도 읽힌다
      .replace(/AI 합성/g, 'AI')         // 형제 짝 = 같은 슬롯에 들어가는 두 값이라 한쪽만 줄이면 문법이 갈린다
      .replace(/오버레이|자막/g, 'OVL'); // 가장 긴 낱말 = 이 자리 최대 절약(저장 라벨은 「자막(…)」 문법이라 둘 다 받는다)
  } };
})();
