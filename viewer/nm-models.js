...
window.NM_MODELS = {
  ...
  // ⚠ 아래 둘 = **어느 판으로 찍었는지**를 화면이 그대로 말하게 하는 키(산출 `video.json.lane`).
  //   한 통로가 프리셋에 따라 두 판을 쓰므로 통로 이름 하나로는 화면이 거짓말을 한다(2.5로 찍고
  //   2.0이라 부르는 상태). 2.5 승격 조건이던 「가용성 실측」 = 260812 창구 실측(모델 목록 실존 ·
  //   길이 4~30초 · 480p·720p · 30초 견적 195크레딧 · 크레딧 소모 0).
