#!/usr/bin/env bash
# 클라우드 액션 레인 겉옷 — 구글 드라이브 「내 드라이브/action」 폴더를 우체통 삼아 PC 레인을 돌린다
#   (운영자 260814 «git 액션 대체 기능을 클라우드 폴더에 · 항시 돌게 · 윈도우/맥 둘 다 ·
#    G: 는 변화할 수 있고 계정값 = ems1130g@gmail.com» · 요구사항 Q1482~Q1485).
# 몸통은 새로 만들지 않는다 — 실제 일(수집 + 속보 판정 + 경중 채점)은 pc_lane.sh 정본을 그대로 실행(사본 0).
# 이 파일의 몫 3가지:
