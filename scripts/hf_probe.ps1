...
function Done([int]$rc) {
  # ⚠ 기록 저장 자체를 감싼다 — 백신·클라우드 동기화가 바탕화면 쓰기를 막으면, 실패 사유를
  #   알리려던 이 함수가 사유를 말하기도 전에 죽고 창이 그냥 닫힌다(정본 SaveLog 계약 계승).
  try { $Lines -join "`r`n" | Set-Content -Path $LogPath -Encoding UTF8 } catch {
    Write-Host ("기록 파일을 저장하지 못했습니다: " + $_.Exception.Message) -ForegroundColor Yellow
  }
...
# ⚠ **서버가 뭐라고 거절했는지**를 읽는다(정본 = grok_probe.ps1 Web()). 파워셸 5.1 과 7 이 실패
#   응답을 다르게 넘겨서, 한쪽만 읽으면 남는 건 "(400) 잘못된 요청" 같은 껍데기뿐이고 서버가
#   보낸 사유(invalid_grant · 차단 본문)가 통째로 사라진다 = 이 판정기의 존재 이유가 사라진다.
function ErrText($e) {
  $t = "$($e.Exception.Message)"
  try { if ($e.ErrorDetails -and $e.ErrorDetails.Message) { return [string]$e.ErrorDetails.Message } } catch {}
  $resp = $null
  try { $resp = $e.Exception.Response } catch {}
  if ($resp) {
    try {
      $sr = New-Object IO.StreamReader($resp.GetResponseStream())
      $b = $sr.ReadToEnd(); $sr.Close()
      if ($b) { return ("HTTP " + [int]$resp.StatusCode + " · " + $b) }
    } catch {}
    try { return ("HTTP " + [int]$resp.StatusCode + " · " + $t) } catch {}
  }
  return $t
}
...
try {
  ...
  Say "❌ 프로그램 등록 실패." "Red"; Say ("   사유: " + (ErrText $_)) "Red"; Done 1
}
...
$code = $null; $deadline = (Get-Date).AddMinutes(10)
# ⚠ 이 루프 안은 **브라우저가 마음대로 끊을 수 있는 구간**이다(미리 열어보는 연결·먼저 닫기).
#   감싸지 않으면 정확히 승인하는 그 순간에 창이 사유 없이 닫히고 기록도 안 남는다.
#   ⚠ 대기 10분 — 2단계 인증·로그인이 끼면 5분은 빠듯하다.
while ((Get-Date) -lt $deadline -and -not $code) {
 try {
  ...
  if ($first -match 'GET\s+(\S+)') {
    ...
    if ($q -match '[?&]error=([^&\s]+)') {
      Say ("❌ 승인이 거부됐습니다: " + [Uri]::UnescapeDataString($Matches[1])) "Red"
      Say "   같은 계정으로 다시 실행하거나, 브라우저에서 힉스필드에 로그인한 뒤 다시 해 보세요."
    }
    if ($q -match '[?&]state=([^&\s]+)') {
  ...
  $cl.Close()
 } catch { Start-Sleep -Milliseconds 200 }   # 끊긴 연결 하나로 판정기를 죽이지 않는다(다음 연결을 기다린다)
}
...
try {
  ...
  Say "❌ 열쇠 교환 실패." "Red"; Say ("   사유: " + (ErrText $_)) "Red"; Done 1
}
...
try {
  ...
  Say ("   ⚠ 확인 호출이 실패했습니다: " + (ErrText $_)) "Yellow"
}
