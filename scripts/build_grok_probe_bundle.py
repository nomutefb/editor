#!/usr/bin/env python3
# ═══════════════════════════════════════════════════════════════════════════════
# build_grok_probe_bundle.py — grok_probe.ps1 을 '더블클릭 .bat' 하나로 묶는 생성기
...
#
# 사용:  python3 scripts/build_grok_probe_bundle.py          (생성)
#        python3 scripts/build_grok_probe_bundle.py --check  (레포 산출물이 최신인지 · rc=1이면 낡음)
# ═══════════════════════════════════════════════════════════════════════════════
import base64
import sys
...
SRC = ROOT / "scripts" / "grok_probe.ps1"
OUT = ROOT / "scripts" / "노뮤트_그록자격_확인.bat"
...
def main() -> int:
    ...
    if "--check" in sys.argv:
        if not OUT.exists() or OUT.read_bytes() != data:
            print(f"⚠️ 낡음 — {OUT.name} 재생성 필요: python3 scripts/build_grok_probe_bundle.py")
            return 1
        print(f"✅ {OUT.name} = grok_probe.ps1 최신 반영")
        return 0
