...
if [ "${MODE:-}" = "repost" ]; then
  ...
  code=$(curl -s -o /dev/null -w '%{http_code}' --max-time 40 ${AH[@]+"${AH[@]}"} -X POST "${LIVE_BASE:-https://edit.nomute.kr}/api/thumb" -H 'Content-Type: application/json' --data @/tmp/thumb_repost.json 2>/dev/null)
