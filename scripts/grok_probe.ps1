...
foreach ($m in $order) {
  ...
  if ($r.code -eq 200 -and $r.obj) {
    ...
    Say ""
    Say "  >> 넣고 나면 바탕화면 [그록열쇠_붙여넣기.txt] 와 [그록토큰.json] 은 **지워라.**"
    Say "     기기를 옮겨 다니면 이 파일은 어차피 안 따라오고, 남겨두면 열쇠가 그 컴퓨터에 그냥 굴러다닌다."
    Say "     진짜 보관처는 깃허브 비밀값 한 곳이다 - 파일을 지워도 다음 발사는 멀쩡하다."
    Say "     (지웠다면 다음에 이 판정기를 돌릴 때 로그인 창이 한 번 뜬다 = 정상)"
    Say ""
    Say ("-" * 58)
