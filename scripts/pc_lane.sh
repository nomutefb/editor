...
ST=""; ST_BAD=0
_m(){ # $1=이름 $2=rc
  # 실패는 **종료번호까지** 남긴다 — 124 = 시간초과(느린 회선·큰 작업), 그 밖 = 그 스크립트가 스스로 낸 오류.
  # 번호가 없으면 「죽었다」까지만 알고 왜 죽었는지는 그 컴퓨터 로그를 열어야만 알 수 있다(첫 실운영에서 실제로 그 벽에 부딪혔다).
  if [ "${2:-0}" = "0" ]; then ST="$ST $1=ok"; else ST="$ST $1=FAIL$2"; ST_BAD=1; fi
}
...
if _haveclaude; then
  ...
  if [ "${nb:-0}" != "0" ] && [ "$(_left)" -ge 240 ]; then
    BREAKING_MODEL="${BREAKING_MODEL:-claude-opus-5-5}" BREAKING_EFFORT="${BREAKING_EFFORT:-max}" \
  ...
  if [ "${ng:-0}" != "0" ] && [ "$(_left)" -ge 420 ]; then
    GATE_MODEL="${GATE_MODEL:-claude-opus-5-5}" GATE_EFFORT="${GATE_EFFORT:-max}" \
