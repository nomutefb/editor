#!/usr/bin/env bash
# summary_repair.sh — 뉴스요약 제한 보정: 근거 안에서 1회 재작성 (ask.sh·analyze.sh 공용 SSOT)
#
...
summary_repair() {
  ...
  case "$chk" in
    ...
    *)              DIRW="미달 블록에서 빠진 핵심 사실·반론·조건을 기준본과 대조하라. 아직 옮기지 않은 근거 있는 정보가 있을 때만 보강하고, 없으면 짧은 원본을 그대로 돌려줘라";;
  esac
  rprompt="[제한 보정 — 아래 검사 결과의 blocks에 지정된 IG·Thread 코드블록 '내용'만 다시 써라. 다른 블록은 보존한다.]
검사 결과: ${chk}
규칙:
- 기준본 = [자유요약] 코드블록의 사실 본문. ${DIRW} 각 블록의 under/over 사유를 따로 적용한다. IG 600~780자·Thread 370~420자는 정보가 충분할 때의 작성 목표다. IG 상한 800·Thread 보정 상한 430(출처 포함·개행/면책 제외), Thread 게시 하드 500(개행·면책까지 포함)은 지켜라.
- 우선순위 = 핵심 사실·수치 대상/단위/시점·주장 귀속·한계 보존 → 자연스러운 문체 → 분량. 예컨대 단체방 구성원 수를 피해자 수로 바꾸거나 '일부 유출'을 전체 정보 유출로 넓히지 마라. 회사·당사자의 설명은 '~라고 밝혔다/설명했다'로 귀속하고 독립 확인 사실처럼 단정하지 마라. 예정·진행·완료, 의혹·주장·확정도 원문 수준을 유지한다.
- mode=brief_complete는 원본에 선언된 짧은 완결본으로 하한을 채우지 않는다. mode=standard여도 추가할 근거가 없으면 반복·일반론·심리·미래 전망으로 늘리지 마라. 짧다는 사실은 원문 확보가 완전하다는 증거가 아니며, 보정 콜이 누락된 원문을 추정해 메우거나 mode를 바꾸면 안 된다.
- ⛔ 날조 절대 금지: 자유요약의 사실 본문에 없는 **새 숫자·고유명사·날짜·인용·인과 주장**을 도입하지 마라 — 네가 추가하는 모든 문장은 자유요약의 특정 문장으로 소급 가능해야 한다. 제목·출처 날짜·메타데이터·다른 섹션의 숫자는 본문 근거로 재사용하지 마라.
...
- 출력 = 아래 형식 중 검사 결과 blocks에 지정된 섹션만(코드펜스는 \`\`\`text 그대로 · 설명·인사·다른 텍스트 일절 금지):
### [IG — N/800자]
\`\`\`text
(IG 전문)
\`\`\`
