...
summary_repair() {
  ...
  case "$chk" in
    "REPAIR over"*) DIRW="상한을 넘겼다. 문장·📍을 **통째로 덜어** 다음 범위로 줄여라(어미·조사를 깎지 마라 = 체언 종결 유발 · 소속·직함·주어는 깎기 최후순)";;
    *)              DIRW="거기 담긴 사실·수치·인용·맥락을 **더 옮겨 담아** 다음 범위를 채워라";;
  esac
  rprompt="[분량 보강 — 아래 다이제스트의 IG·Thread 코드블록 '내용'만 다시 써라. 다른 어떤 부분도 건드리지 마라.]
규칙:
- 기준본 = [자유요약] 코드블록. ${DIRW} IG 600~780자(상한 800)·Thread 390~440자(상한 450)로 채워라(⚡ 출처 줄 포함해 세는 기준·면책 줄 제외).
