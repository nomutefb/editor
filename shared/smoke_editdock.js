...
(async () => {
  ...
  try {
    ...
    ck('C4 초기 리드백 = 7축(비율·해상도·고프레임·SDR 변환·배경음·음량·컷편집) — 값 축 = 라벨+원본 · 이진 축 = 워드 단독 · 초기 점등 = **배경음·음량 둘**(SDR 변환 = 260913 운영자 제안 승인으로 합류 · 기본 OFF = 무점등 · HDR 원본만 러너가 실변환)(ⓐ 배경음 = 운영자 260803 7차 "배경음 > 활성화가 기본값, 비활성화 하면 > 배경음 제거" = 표시가 뒤집힌 축이라 「기본 상태에서 켜져 보이나」가 곧 그 계약의 회귀 검문 · ⓑ 음량 = 운영자 260712 "음향 기본값 = 보정" ON.anorm 기본 true의 화면 짝 · 나머지 4축은 종전대로 무점등 · 워드 점등 문법 자체는 thumb #cnTog 정본 불변)   ⚠ 5축 → 6축 개정 = 운영자 260810 "일단 1번대로 한번 누르는거로 배선 ㄱ" — 260803 7차가 음량 카드를 지우면서 **끄는 수단까지 같이 사라졌는데**, 260810 소리 봉합으로 재압축이 3회 → 1회가 되어 **남은 유일한 소리 가공이 이 축**이 됐다 → 음질이 의심될 때 그 마지막 하나를 꺼봐야 원인을 가릴 수 있다(기본 ON 자체는 불변 = 평소 동작 100% 종전)', /^비율 원본 \/ 해상도 원본 \/ 고프레임 \/ SDR 변환 \/ 배경음 \/ 음량 \/ 컷 편집$/.test(r1.readback) && r1.onN === 2 && r1.onWords === '배경음,음량', 'rb=[' + r1.readback + '] on=' + r1.onN + '(' + r1.onWords + ')');
