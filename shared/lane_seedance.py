...
#
# ⚠⚠ **모르는 이름을 통과시키지 않는다** — 창구는 모르는 화질을 거절하지 않고 **조용히 기본값으로
#   되돌린다**(260813 실측 = 엉터리 "9k" 도 720p 와 같은 22.5 크레딧 회신 · "2k" 도 22.5 · 반면
#   1080p 45 · 4k 110). 오류가 한 줄도 안 남으므로 「골랐는데 안 바뀐다」가 무증상으로 성립하고,
#   검출기가 사람 눈뿐이 된다. 그래서 표에 없는 이름은 여기서 **멈춘다**(조용히 싼 화질로 나가는
#   것보다 안 나가는 쪽이 싸다 · 구판 `.get(_res, _res)` 는 그 통과가 기본값이었다).
# 값 출처 = 창구 명세 직접 실측(확인 절차 ②-c) — 판마다 아는 이름이 다르다.
#   시댄스 2.5 = 480p·720p (길이 4~30초) / 시댄스 2.0 = 480p·720p·1080p·4k (길이 4~15초)
#   ⚠ **두 판 어디에도 2K 는 없다** — 화면 어휘(SB_CAPS)도 같은 실측을 따른다.
...
if _res and _probe:
    ...
    if _v is None or _v not in _ok:
        raise SystemExit(
            "::error::화질 이름 「{}」 — {} 가 아는 이름이 아니다(아는 이름 = {}). "
            "그대로 보내면 창구가 조용히 기본값으로 되돌려서 고른 값과 다른 것이 나온다.".format(
                _res, PRESET["model"], " · ".join(_ok)))
    PRESET["res"] = _v
...
def _check():
    ...
    # ②-c 이 판이 **실제로 아는 화질 이름**을 창구에 직접 묻는다.
