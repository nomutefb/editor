...
방법 — 「주색 대역의 hue만 타깃으로 치환, 채도·명도는 원본 유지 + 판별 부스트」:
  ...
  · desaturate 스위치 = hue 치환 대신 채도를 눌러 회색으로(260818 = 시스템 축 sys가 이 축을 쓴다).
...
#   sys   = --mut               #8fa697    시스템(파이프라인 이상·라이브 검문·수집 실패) = **무채**
#           (운영자 260818 «파이프라인하고 라이브 검문, 수집 실패 포함시켜서 컬러를 mut으로 · 그래야 이슈가
#           혼자 긴급느낌으로 종류가 맞는듯») — 구 노랑에서 이동. 이유 = 노랑을 ⚡이슈가 쓰기 시작했는데
#           같은 노랑을 시스템 축이 같이 쓰면 **손목·잠금화면에서 둘이 구분이 안 된다**. 시스템 축은 「나만
#           고치면 되는 일」이라 색이 조용한 쪽이 맞고, 노랑은 「지금 봐야 하는 소식」 전용으로 남긴다.
#           ⚠ desaturate = 채도를 눌러 무채로(구 test 슬롯이 쓰던 축 = 부활 · 값 창작 0)
#   trend = --info(--accent-5)  #0FFD02    급상승(핫 키워드) = 상승 초록 — ⚠ **아래 kw(키워드 알림)와 별개 축**
#           (운영자 260818 «별개인거 알지»): trend = 내가 등록한 적 없는데 세상에서 갑자기 뜨는 말 /
#           kw = 내가 직접 등록해둔 말. ⚠ 260818 실측 = trend를 쏘는 코드는 아직 없다(조건 미정 대기).
#   iss   = --warn(--accent-4)  #FFE13D    이슈(긴급은 아닌데 매체가 몰린 건) = 노랑 — 화면 ⚡이슈 배지(.sc-badge.iss
#           = --warn)와 **같은 색**을 쓴다(운영자 260818 «표기를 긴급처럼 노랑으로» · 값 = 그 배지 정본 사본).
#           ⚠ 시스템 경보(sys)와 색이 같다 = 화면 정본이 둘 다 --warn이라 그렇다(새 색 창작 금지 계약).
#           구분은 제목·본문이 한다(이슈 = 「(이슈) 헤드라인」 · 시스템 = 「수집 실패」류).
...
KINDS = {
    ...
    "sys":   {"hex": "#8fa697", "token": "--mut",      "label": "시스템(파이프라인·검문·수집실패)", "desaturate": True},
    "trend": {"hex": "#0FFD02", "token": "--info",     "label": "트렌드"},
    "iss":   {"hex": "#FFE13D", "token": "--warn",     "label": "이슈(매체 몰림)"},
