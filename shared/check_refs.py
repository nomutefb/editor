...
_AFFORD_BASE = {   # 파일별 선존 스냅샷 = **260803 실측값**(추측 금지 — 코드로 세서 박았다) · 초과만 FAIL · 축소 지향
    ...
    'viewer/edit.html': 5,
    'viewer/index.html': 10,   # 260923 래칫 11→10(발행본 .pub-open 행 삭제)
    'viewer/k.html': 1,
    'viewer/ly.html': 2,
    'viewer/thumb.html': 1,
    'viewer/tr.html': 1,
...
_MODEL_NM_BASE = {   # 파일별 선존 표기 스냅샷(260803 실측 — 전부 주석·운영자 원문 인용 · 가시 문자열은 Q1285에서 소거 완료) · 줄면 그만큼 낮춰라(래칫)
    ...
    'viewer/index.html': 33,        # 제미나이·페이블 등(주석 다수) · 260923 래칫 35→33(공유·발행 산출물 원표기 소멸)
...
def _apply_models_mod():
    """경계 가드(인원 접미사·버전 연장)·키 기준 정규식의 단일 정본 = shared/apply_models.py — 사본을 두면 치환기와 게이트가
    갈라진다(260923 실측: 가드가 `(?![인명])`뿐이라 `claude-opus-5` 은퇴 뒤 `claude-opus-5-5`·`Opus 5.5` 전부를 잔존으로 오판)."""
    import importlib.util
    spec = importlib.util.spec_from_file_location('apply_models', os.path.join(os.path.dirname(os.path.abspath(__file__)), 'apply_models.py'))
...
def check_model_ids():
    ...
    for path in _model_scan_files(reg):
        try:
            ...
            continue
        rel = _repo_relpath(path, ROOT)
        for lit in sorted(set(id_re.findall(src))):
            if lit not in ids:
    ...
    # 대행(follow) = 값이 대상 티어와 같아야 하고 1단만(연쇄 대행 = 승격 때 동기가 끊긴다)
    for k, t in tiers.items():
        f = t.get('follow')
        if not f:
            continue
        lead = tiers.get(f)
        if not lead or lead.get('follow'):
            bad.append('models.json: [%s] 대행 대상 `%s` 부재 또는 연쇄 대행(1단만 허용)' % (k, f))
        elif (t['id'], t.get('en'), t.get('ko')) != (lead['id'], lead.get('en'), lead.get('ko')):
            bad.append('models.json: [%s] 대행인데 값 %s ≠ [%s] %s — `python3 shared/apply_models.py %s --follow %s` 재실행'
                       % (k, (t['id'], t.get('en')), f, (lead['id'], lead.get('en')), k, f))
        if not t.get('원래'):
    ...
    if bad:
        ...
        for b in bad: print('   -', b)
        return 1
    fol = ['%s→%s' % (k, t['follow']) for k, t in tiers.items() if t.get('follow')]
    print('✅ 모델 ID 게이트 — %d티어(%s) + 벤더 %d종(%s) 정본 일치 · 구세대 %d종 잔존 0 · 대행 %s(스캔 %d파일 · 승격 = apply_models.py).'
          % (len(tiers), ' · '.join(t['id'] for t in tiers.values()), len(vendors),
             ' · '.join(k for k, *_ in vendors), len(retired), ' · '.join(fol) or '없음', len(_model_scan_files(reg))))
    return 0
...
def check_nm_sync():
    ...
    try:
        ...
        return 1
    for lit in ("'/nm-sync.js?_='", '/?nosw=1', "method: 'HEAD'"):
        if lit not in msrc:
...
_LAND_BASE = {   # 260816 실측 스냅샷 — 회수 경로도 실패 표기도 없이 조용히 끝나는 인라인 착지.
    # ⚠ **8건 전건 해소(260816 일괄)** — 제작 6곳은 `exit 1`(과금 산출이 유실되면 화면이 「제작중」에 멈춘다) ·
    #   관측 2곳은 회수 경로 명시(데이터 churn 이라 다음 회차가 덮는다 = 실패로 잡을 자리가 아니다).
...
def check_canon_host():
    ...
    술어 = **스킴이 붙은 절대 주소만** 위반(https://apps.nomute.kr).
      ⚠ 이 한 줄이 실효 조건 = 스킴 유무가 「부르는 곳」과 「비교하는 곳」을 정확히 가른다.
        ...
        요청 출처 허용 목록(functions/api/*.js originOk 3곳) · 서비스워커 CANON_HOSTS ·
...
def check_focus_contract():
    """요약 보강 = 원문의 관점 축을 지킨다(하드 · 260824 실사고 봉합 · check_orig_title_restore 의 형제).

    계약 = 「전문(사전 추출 본문·붙여넣기)이 주어진 요약에서 보강 모드는 관점 축 4계약을 동반한다」
       — ① 초점 보존(헤드·리드·결론 = 원문 제목·리드의 초점) ② 보강 종속(원문이 세운 문단 축 안에만)
       ③ 사건 동일성(원문의 인물+시점+장소 일치 시만 채택) ④ 검색 상한 = 천장이지 할당량이 아니다(0회 정상).

    ⚠️ 실사고 = 260824 운영자가 「대통령, 경찰 개혁 지시」 리드의 전문을 붙여넣었는데(그 기사의 스탠스가
       좋아서 고른 것) 산출은 「실종 신고가 접수된 그날 사건은 닫혔다」로 열어 경찰 부실수사 스캔들 전체를
       새 뼈대로 재구성했다(타매체 재검색 · 총리·직무대행·석방·298건 = 전부 원문 밖 새 축 · 대통령 지시는
       둘째 문단으로 강등). 운영자 «원래 내용이 가지고 있던 포커싱, 스탠스, 말하고자 했던 방향 이런게 모두
       무시» + 문제 3축 = 초점 무시 > 동명 딴 사건 위험 > 재검색 시간.
    ⚠️ 진범 = 보강 모드 계약(260803 「보강이 기본값」)이 전문을 「'사실의 축'」으로만 정의 — 관점·초점의
       축이라는 말이 세 표면 어디에도 없었고, 보강 우선순위 ③(사건 경위·이전 보도 맥락)이 재구성을 사실상
       초대했다. 보강 자체는 운영자 260803 값이라 보존(이번 지시 = 폐지가 아니라 종속화 · 극단 = 기존
       「원본 한정」 프리셋 그대로).
    ⚠️ 신설 사유 = 기존 게이트는 전부 다른 축 — `check_orig_title_restore` = **제목 필드** · `check_paste_url_stamp`
       = **원문 주소** · `check_style_ratchet` = 본문 **문체** → 「요약이 원문의 관점을 지키는가」는 축 자체가
       없었고, 계약이 프롬프트 문구뿐이라 한 표면만 개정돼도 조용히 갈린다(이 레포 최빈 미러 드리프트).
    판정 = 3표면 계약 문구 생존 · 정적 · 렌더·LLM·네트워크 0 · **면책표 없이 하드 0** · .sh 는 실행줄
       판정(주석 위장 차단 = _has_exec_line 계승). ⚠️ 한계 = 문구 생존까지다 — 산출이 실제로 초점을
       지키는지는 모델 준수 축(「초점」은 결정론 판정 술어가 없다 = check_orig_title_restore 의 결정론
       복원 같은 짝이 원리적으로 불가 · 다음 실산출이 판정).
    """
    bad = []
    pr = os.path.join(ROOT, 'prompts', 'news-analysis.md')
    an = os.path.join(ROOT, '.github', 'scripts', 'analyze.sh')
    ...
    for needle, why in (('관점의 축', '정본 계약 머리(전문 = 사실의 축이자 관점의 축)'),
                        ...
                        ('0회도 정상', '검색 절약 술어(상한 = 천장이지 할당량이 아니다)')):
        if needle not in pt:
    ...
    if bad:
        print('❌ 관점 축 계약 —', ' / '.join(bad))
        return 1
    print('✅ 관점 축 계약 — 전문 요약의 보강이 원문 초점·사건 동일성·검색 절약 계약을 3표면에서 유지.')
    return 0
...
def check_edit_track_chain():
    ...
    try:
        ...
        return 1
    # ⚠ ⑧⑨는 **주석을 걷어낸 코드부**에서만 판정한다 — 이 봉합의 처방 주석이 판정 문자열(`d.xtr_note`·
...
def check_img_upsize():
    ...
    try:
        ...
        try:
            try:
                import yaml as _yaml
            except ImportError:   # 형제 게이트와 같은 처리(pyyaml 없는 기기 = 이 축만 건너뜀 · CI 는 설치 = 판정 유지)
                _yaml = None
                print('⚠ check_img_upsize ②-d: pyyaml 없음 — moreimg 발사 입력 계약 스킵')
            _on = (_yaml.safe_load(yt) or {}) if _yaml else {}
            ...
            for _rel in (('.github/workflows/news-analyze.yml', '.github/workflows/news-ask.yml',
                          '.github/workflows/moreimg.yml', 'functions/api/moreimg.js') if _yaml else ()):
            ...
            bad.append('moreimg 발사 입력 계약 검사 실패: {}'.format(str(e)[:160]))
...
def main():
    ...
    try:
        ...
        if check_paste_url_stamp() != 0:   # 전문 붙여넣기 카드 = 원문 주소가 화면까지 오는가(260817 실사고 — url이 비면 요약 모달 좌상단 원문 버튼이 회색으로 죽는다 · 화면 증상 = 버튼이 떼어진 것처럼 보임)
            rc = 1
        if check_focus_contract() != 0:   # 요약 보강 = 원문 관점 축 4계약 문구 생존(260824 실사고 — 보강이 원문 초점·스탠스를 갈아엎고 딴 뼈대로 재구성 · 3표면 중 한쪽만 개정되면 조용히 갈린다)
            rc = 1
