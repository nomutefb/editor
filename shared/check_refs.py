...
def check_edit_track_chain():
    ...
    checks = [
        ...
        ('⑧ 실패 사유 화면 도달', 'd.xtr_note' in vwc),   # ⚠ 260816 실사고 = 러너가 6곳에서 쓰는 xtr_note 를 **뷰어가 한 곳에서도 안 읽고 있었다**(실측 = viewer/ 전수 grep 코드 0건 · 실물 데이터엔 「keying 처리에 실패해서 그 단계는 빠졌어」가 박혀 있다) → 가림·키잉·크로마키가 통째로 빠져도 화면엔 「완료」만 떠서 운영자 눈이 유일한 검출기였다. ①~⑦은 전부 「옵션이 러너까지 **도달하는가**」(체인 생존) 축이라 「그 결과가 **실패했을 때 그걸 말하는가**」는 축 자체가 없었다.
        ('⑨ 알파 표시 계승', all(k in vwc for k in ('.vstage.alpha', "classList.toggle('alpha'", "classList.remove('alpha')"))),   # ⚠ 260816 실사고 = 키잉은 실제로 동작하는데(실측 알파 평균 69/255 = 배경 73% 제거) `.vstage`·`.vstage video` 가 순흑이라 **배경이 빠진 자리를 검정이 그대로 메워** 원본과 구분이 불가능했다. 정본은 이미 `viewer/track.html`(178~179·905·837행)에 있었고 **편집 탭만 계승을 안 했다** = 이 레포 최빈 축 「형제는 가진 걸 자기만 안 가졌다」. 3부품 = CSS 규칙 · 완료 시 토글 · 대기 진입 해제(마지막이 빠지면 다음 비알파 제작에 지난 회차 체커보드가 남는다).
    ]
    for name, ok in checks:
        if not ok:
