...
# claude_effort_down(노력도): 타임아웃(rc=124) 재시도용 노력도 한 단계 하향 사다리(260912 · ask.sh 본선).
#   max→high · high→medium · 그 아래·미지 값 = 그대로(더 내릴 단이 없다 = 호출부가 종전 계정 전환 폴백 판정).
#   왜 계정이 아니라 노력도인가: ask 경로 계측 181건 실측 = 소요시간이 출력 토큰과 상관 0.99(=사고 토큰이 시간이다) ·
#   600s 초과 실패 4건이 전부 max 기간(high·medium 97건 0건) · 260912 실사고 = max 600s 초과 → 서브계정 max 재시도
#   → 또 600s 초과 = 21분 실패. 입력바운드 타임아웃은 계정을 바꿔도 반복하고(260704 평의회 자인) 노력도를 내려야 끝난다.
#   ⚠ 사다리 바닥 = medium(뉴스 요약은 사실 검사 규칙이 무거워 low 는 날조 위험 축 · 운영자 선택 밖).
claude_effort_down() {
  case "${1:-}" in max) printf 'high';; high) printf 'medium';; *) printf '%s' "${1:-}";; esac
}
