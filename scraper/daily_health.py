...
def buried_alert():
    ...
    # ⚠️⚠️ 260818 2차 — 발화 축 = **같은 사건 묶음(group_id)이 확정된 건만**(운영자 「문제가 아닌게 문제
    #   해결알림을 띄우는게 이상함」 = 비문제 상시 발화 금지). 같은 날 12:12 발화분 14건 전수 판독 실측 =
    #   ⓐ 조각 2(거제 폭우 — 같은 사건이 cross 21로 이미 화면 최상단 = 묻힘 아님) ⓑ 외신 원문 3(BBC·알자지라
    #   — 한국 매체 반향[cross] 축과 원리적으로 안 이어져 진입 구조 자체가 없음) ⓒ 나머지 = 단독 저cross 국제
    #   단신 → 전건 「조치할 코드 자리가 없는」 항목이고, 그 상시값(실측 12~17)이 발화선 5를 매일 넘어
    #   비문제가 매일 울렸다(연예 예외와 같은 병 = 조치 불가분이 발화선을 밀어올려 진짜 신호를 가린다).
    #   ⚠ 묶기없음 건은 「화면에 이미 오른 사건의 조각」과 「진짜 단독 묻힘」을 기계가 못 가른다(낱말 병합
    #     금지 계약 · 260625 안산↔청주) — 못 가르는 값은 발화 근거가 못 된다 → 발화는 AI 묶음이 확정돼
    #     「형제 합산으로도 미진입」이 사실로 검증된 건만 센다(이 알림 본문이 원래 주장하던 그 술어의 기계화).
    #   단독·조각·외신은 계측·본문 참고로만 = 숨김 0(grade2 참고치·연예 분리 관용구 3번째 계승).
    #   롤백 = BURIED_GROUPED_ONLY=0(watchdog.yml 배선 · 종전 = 비문화 전건 발화 축).
    grouped_only = (os.environ.get("BURIED_GROUPED_ONLY", "1").strip().lower()
                    not in ("0", "false", "no"))
    cul = [x for x in g3 if (x.get("cat") or "") == "문화"]             # 연예 예외 조항 [3] = cross 구조적 미달
    nc = [x for x in g3 if (x.get("cat") or "") != "문화"]
    solo = [x for x in nc if not x.get("group_id")] if grouped_only else []   # 묶음 미확정 = 조각/단독 판별 불가
    core = [x for x in nc if x.get("group_id")] if grouped_only else nc      # 발화 축 = 묶음 확정 후에도 미진입
    print(f"· 묻힘(전 카테고리 4h+ 누적 미진입): grade3 발화축 {len(core)}건"
          f"(원자료 {len(raw3)}건 · 연예 예외 {len(cul)}건 · 묶음 미확정 {len(solo)}건 별도)"
          f" · grade2 {g2}건 (발화선 g3≥{warn_at})")
    ...
    lines = [f"묻힌 대형(grade3) {len(core)}건 — 4시간이 지났는데 누적 칼럼에 못 들어간 사건입니다."
             ...
             f"✅ 화면 병합을 재현한 뒤의 숫자입니다(원자료 {len(raw3)}건 → 같은 사건 묶기 반영"
             f" {len(core) + len(cul) + len(solo)}건 → 연예 {len(cul)}건·묶음 미확정 {len(solo)}건 분리 후 {len(core)}건)."
    ...
    lines += ["", "진입 자격 = cross≥8 OR 긴급 OR followEnters(cross≥4 ∧ [rc≥6 OR rc≥5+grade≥2 OR rc≥3+강지문]).",
              ...
              (f"⚠️ 이 중 {len(nogrp)}건은 같은 사건 묶기가 아직 안 붙었습니다(위 「묶기없음」 표시)."
               ...
               "⚠️ 위 목록은 전건 같은 사건 묶음이 확정된 건 = 형제 합산까지 반영된 실제 묻힘입니다."),
              (f"ⓘ 이 밖에 묶음 미확정(단독·조각·외신 원문) {len(solo)}건은 발화 판정 밖입니다"
               "(260818 · 조각/단독 판별 불가 축 = 계측으로만 추적)." if solo else ""),
