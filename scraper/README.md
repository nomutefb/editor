...
- 정본 = `feeds.csv`(177피드). 죽은 피드는 러너 원장 `obs/feed_health.json`(dead·zombie)과 라이브 응답·최신 항목 나이로 판정한다.
...
- 260923 3차: 노컷 신주소(`rss.nocutnews.co.kr`) 사회·연예·전체 복귀 · 연예(SBS연예·JTBC연예·스포츠조선·스포츠경향) · 테크(ZDNet코리아·IT조선·아이뉴스24 · 블로터는 금융·부동산이 과반이라 `_all_`) · 사회(SBS·JTBC 섹션). 섹션 행은 같은 매체 종합 행보다 앞(같은 기사는 첫 피드 섹션을 단다). 텐아시아 TOPIC 은 나이·외모 낚시가 과반이라 제외.
- 카드 분류 = 묶음 멤버 피드 섹션 다수결(`to_candidates.cluster_sec` · 대표가 섹션 없는 피드면 키워드·AI 판정).
- 날짜: 표준 파서가 못 읽는 국내 표기(쉼표 뒤 공백 없음·숫자 월·압축형)는 `parse_time` 폴백이 읽는다(발행시각 없는 기사 20%→6%). 남은 무시각 = 한겨레처럼 항목에 날짜가 아예 없는 피드.
- 교차 셈 = 원 보도국 기준. `CROSS_ALIAS`(연합뉴스TV→연합 · NYT코리아→NYT = 재송출·같은 보도국만 · 같은 그룹 별개 보도국은 따로) + `REPRINT_AUTHOR`(조선일보 피드 속 OSEN·스포츠조선·뉴시스 기사 = author 칸 원 매체로).
- 미확보(피드 없음·403): KBS · YTN · 뉴스1 · 한국일보 · 중앙일보 · 한경(한국일보 `rss.hankookilbo.com`·뉴스1·한경은 폰 레인에서 재확인 여지).
