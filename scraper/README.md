...
- 정본 = `feeds.csv`(178피드). 죽은 피드는 러너 원장 `obs/feed_health.json`(dead·zombie)과 라이브 응답·최신 항목 나이로 판정한다.
...
- 260923 3차: 노컷 신주소(`rss.nocutnews.co.kr`) 사회·연예·전체 복귀 · 연예(SBS연예·JTBC연예·스포츠조선·스포츠경향·텐아시아) · 테크(ZDNet코리아·블로터·IT조선·아이뉴스24) · 사회(SBS·JTBC 섹션). 섹션 행은 같은 매체 종합 행보다 앞(첫 피드가 분류를 정한다).
- 날짜: 표준 파서가 못 읽는 국내 표기(쉼표 뒤 공백 없음·숫자 월·압축형)는 `parse_time` 폴백이 읽는다(발행시각 없는 기사 20%→6%). 남은 무시각 = 한겨레처럼 항목에 날짜가 아예 없는 피드.
- 교차 셈 계열 합산 = `CROSS_ALIAS`(연합뉴스TV·MBN·스포츠조선·IT조선·스포츠경향 → 모회사).
