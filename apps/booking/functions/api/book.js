...
//   booking/stock.json         재고   { labels:{a:"A"}, cap:{ a:{double:5,twin:5}, ... } }
...
const DEFAULT_LABELS = { a: 'A', b: 'B', c: 'C' };
...
async function reserve(env, p4, name, g, t, cap) {
  ...
  const at = Date.now();
  const rec = { p4, name: name || '', group: g, type: t, typeName: TYPE_NM[t], at };
...
// 한 줄에 「뒷4자리, 그룹, 이름」. 구분자는 쉼표·탭·세미콜론 아무거나. 이름은 없어도 된다.
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (op === 'auth') {
      ...
      if (!p4ok(p4)) return J({ ok: false, code: 'badp4', msg: '숫자 4자리를 입력해줘.' });

      const roster = await readJson(env, 'roster.json', {});
      const who = roster[p4];
      if (!who) return J({ ok: false, code: 'notfound', msg: '예매자 명단에 없는 번호야. 번호를 다시 확인해줘.' });

      const { cap, labels } = await getStock(env);
      const mine = await readBooking(env, p4);
      if (mine) return J({ ok: true, done: true, name: who.n || '', label: labels[who.g] || '', booking: mine });

      const all = await listBookings(env);
      return J({
        ok: true, done: false, name: who.n || '', label: labels[who.g] || '',
        left: leftOf(cap, all, who.g), typeName: TYPE_NM,
      });
    }
    ...
    if (op === 'book') {
      ...
      const roster = await readJson(env, 'roster.json', {});
      const who = roster[p4];
      if (!who) return J({ ok: false, code: 'notfound', msg: '예매자 명단에 없는 번호야.' });

      const { cap, labels } = await getStock(env);
      const r = await reserve(env, p4, who.n, who.g, t, cap);
      if (!r.ok && r.code === 'soldout') {
        const all = await listBookings(env);
        return J({ ok: false, code: 'soldout', msg: '방금 마감됐어. 남은 자리로 다시 골라줘.', left: leftOf(cap, all, who.g) });
      }
      if (!r.ok) return J({ ok: false, code: r.code, msg: '이미 선택을 마쳤어.', booking: r.booking, label: labels[who.g] || '' });
      return J({ ok: true, booking: r.booking, label: labels[who.g] || '' });
    }
    ...
    if (op === 'admin_get') {
      const [roster, { cap, labels }, all] = await Promise.all([
      ...
      const left = {};
      for (const g of Object.keys(cap)) left[g] = leftOf(cap, all, g);
      return J({ ok: true, cap, labels, left, rows, rosterCount: Object.keys(roster).length, typeName: TYPE_NM });
    }
    ...
    if (op === 'admin_stock') {
      ...
      await env.R2.put(PFX + 'stock.json', JSON.stringify({ cap, labels }), {
        httpMetadata: { contentType: 'application/json; charset=utf-8' },
      });
      return J({ ok: true, cap, labels });
    }
