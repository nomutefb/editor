// pages.dev 접속을 커스텀 도메인(edit.nomute.kr)으로 강제 리다이렉트.
...
// + 260815 코워크 확장 2건:
//   ① 정본 도메인을 env(CANONICAL_HOST)로 — 백업 판이 같은 미들웨어를 그대로 쓴다.
//      ⚠ 미설정 폴백 = edit.nomute.kr(260821 운영자 «옛 거는 아예 안쓰게») — 구 폴백은 옛 화면 apps.nomute.kr 이었고,
//      그게 미리보기 배포(CANONICAL_HOST 미주입)에서 실제로 발동했다(실측 260821 = 가지 미리보기 /thumb.html 이 301로 옛 화면행).
//      옛 화면은 옛 계정 배포라 새 저장소 커밋을 영영 안 받는다 = 그리 보내면 몇 달 전 화면이 뜨는 자리.
//      ⚠ 이 줄은 스킴 없는 호스트 문자열이라 check_canon_host 술어(절대 주소만 위반)를 구조적으로 비껴간다
//      = 게이트 사각이었고 운영자가 미리보기를 눌러야만 드러났다(insta-thumb-miss 동축).
...
async function putLive(env, kind, j, ctx) {
  try {
    ...
    const rec = {
      ...
      bid: (typeof j.bid === 'string' && /^[A-Za-z0-9-]{1,40}$/.test(j.bid)) ? j.bid : undefined,   // 묶음키(260913 · 발사 API가 에코한 것만) — 한 번의 「생성」이 요청 N개(작업 id N개)로 나갈 때 다른 기기가 형제를 한 잡으로 합치는 단서(thumb 배치 = 헤더+자막)
      bi: (Number.isInteger(j.bi) && j.bi >= 0 && j.bi < 16) ? j.bi : undefined,   // 묶음 안 순서(헤더 0 · 자막 1)
...
export async function onRequest(context) {
  ...
  if (url.hostname.endsWith('.pages.dev')) {
    ...
    url.hostname = (context.env && context.env.CANONICAL_HOST) || 'edit.nomute.kr';   // 폴백 = 정본 화면(위 머리 ①) — 옛 화면으로 새던 유일한 살아있는 경로였다
