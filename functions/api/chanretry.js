...
export async function onRequestPost({ env, request }) {
  ...
  try {
    ...
    // 발사 = GitHub 5xx(그쪽 일시 장애)만 1.2s 뒤 1회 재시도(260817 · snsretry 미러 동반) — 4xx(권한·비활성·경로)는 재시도 무익 = 즉시 사유 반환.
    const fire = () => GH(env.GH_TOKEN, `actions/workflows/${wf}/dispatches`, 'POST', inputs ? { ref: REF, inputs } : { ref: REF });
    let r = await fire();
    ...
    //   사유가 화면까지 오도록 우리 JSON으로 받는다(실패 사유 화면 도달 계약 · snsretry 미러 동반).
