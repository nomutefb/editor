...
export async function onRequestPost({ request, env }) {
  ...
  const SB_SET = {
    ...
    '화풍': ['자동', '극화', '수채', '실사', '만평', '시네마틱', '일러스트', '3D', '픽토', '레고'],   // '자동'(운영자 260816) = 감독이 연출 목적·자료를 읽고 고른다 — 아래에서 **고를 목록을 같이 실어 보낸다**(감독이 우리 화풍 이름 밖으로 나가면 나중에 그 화풍으로 고정할 수가 없다)
  ...
  for (const k of Object.keys(SB_SET)) {
    ...
    // 화풍=자동 → 「고를 것」 목록을 **이 화이트리스트 그대로** 붙인다(사본 0 = 목록이 늘면 지시도 같이 는다).
    //   ⚠ 목록을 안 주면 감독이 '수묵 담채' 같은 우리 목록 밖 이름을 지어낸다 — 그러면 운영자가
    //     그 결과를 마음에 들어해도 화면 레일에서 같은 값을 다시 고를 수가 없다(재현 불가).
    //   ⚠ 세부 화풍 목록은 **일부러 안 붙인다** = 어느 세부가 어느 화풍 소속인지는 이 서버가 모르고
    //     (매핑 정본은 뷰어 nm-styles.js), 평평한 30개를 그냥 주면 화풍과 안 맞는 세부가 뽑힌다.
    //     화풍을 고른 뒤의 세부는 sb-make.md 의 「그 화풍 안에서 감독 재량」 계약이 이미 덮는다.
    if (k === '화풍' && v === '자동') { pairs.push('화풍=자동(고를 것: ' + SB_SET['화풍'].filter(x => x !== '자동').join('·') + ')'); continue; }
