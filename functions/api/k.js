...
export async function onRequestPost({ request, env }) {
  ...
  };   // ⚠ 비율·화질·프레임·표현1·표현2·세부 축 회수(운영자 260825 — 상세 사유 = viewer/k.html K_VALS 주석 · 구 클라이언트가 보내오는 회수 축 값은 이 화이트리스트가 자연 폐기 · 2면 동기 = check_k_models)
  const REROLL_AXES = ['카메라', '조명', '액션', '오디오'];   // 화이트리스트(뷰어 리롤 버튼·k-make 룰과 1:1 · 화풍 = 260825 회수{표현 축 폐지 동반 — 이미지 시작 워크플로에서 화풍 재추첨은 시작 이미지와 충돌})
  ...
  const RR_DROP = { '오디오': ['오디오'] };   // (260825) 화풍 항목 회수 — 화풍 리롤·표현 축이 함께 폐지돼 드롭할 고정 설정이 없다(구 계약 = 화풍 리롤 시 표현1·2·세부 동반 드롭 · 260710)
