...
export async function onRequestPost({ request, env }) {
  ...
  }
  {
    ...
    if (app === '2' && params.mode === 'header' && params.tpl === 'jinjja') {
      ...
      // ⚠ 260818 Q1518/Q1519 = 구판은 opa 1개일 때 `out.{ext}`로 떨어뜨렸는데, 파일명→변형라벨 매핑(thMetaLabel·build-viewer thLabel)이
      //   `out`을 '기본'으로 뭉개서 기기간 복원·이전 제작 캡션에서 OPA가 사라졌다(릴스는 항상 opa{o}라 무사 · 배경합성 out.jpg도 동일 증상).
      //   → 릴스와 같이 opa{o}로 통일하면 신규 산출은 파일명이 OPA를 실어 프레시·복원 어디서나 OPA60이 뜬다(러너 thumb-make.yml emit도 짝 변경).
