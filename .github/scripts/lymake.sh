...
tone = o.get("tone") or "sns"
if tone == "plain":
    L.append("- 톤: SNS 감성 의역 최소화 — 담백한 직역체.")
elif tone == "talk":
    # 의역 강도 상단(운영자 260812 "의역의 강도를 좀 더 창의적으로, 사람이 말하는것 처럼") — 'sns'(지침 기본) 위 한 단.
    #   ⚠ 새 규칙을 창작하지 않는다: 지침 [의역 철학] ■ SNS 감성 2패스의 **패스 2를 한 번 더** 돌리라는 지시 + 그 절의
    #   가드 문장(의미 변형·없는 뉘앙스·과한 인터넷 말투·원문보다 길어지는 의역 금지)과 [톤 스위칭 원칙] 우선순위를 그대로 인용해 못박는다.
    #   가드를 같이 안 실으면 강도만 올라가 날조·오버액션으로 샌다(프롬프트 자기모순 = thumb 프롬프트 봉합 교훈).
    L.append("- 톤: 의역 강도 **최상**. 지침 [의역 철학] SNS 감성 2패스의 **패스 2를 한 번 더** 돌린 결과를 낸다 —")
    L.append("  원문 어순·번역체 수식을 버리고, 그 상황에서 **한국 사람이 실제로 입으로 말하듯** 다시 쓴다.")
    L.append("  자문: 이 문장을 한국인이 카메라 앞에서 말했다면 정말 이렇게 말했겠는가. 아니면 다시 쓴다.")
    L.append("  단 가드는 그대로 — 의미 변형·없는 뉘앙스/사실 추가 금지 · 과한 인터넷 말투(~거임 류) 금지 ·")
    L.append("  원문보다 길어지는 의역 금지 · [톤 스위칭 원칙](연설·뉴스·역사 서술 = 담백 사실체)이 이 강도보다 우선.")
