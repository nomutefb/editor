...
# ⚠ **손으로 적지 않고 달러 실측에서 뽑는다**(운영자 260813 2차 「원은 그냥 가정한거야 · 달러에
#   1.5를 곱해서 환율 환산하면된다」 = 1달러 1,500원). 구판은 195 를 그대로 박아 놨는데, 그러면
#   달러 단가가 바뀌어도 원이 안 따라와 두 숫자가 조용히 갈린다 — 곱셈 한 줄로 묶는다.
# ⚠ 손대는 곳은 **환율 하나**다(달러 단가는 실측이라 우리가 정하는 값이 아니다).
#   레포 변수 USD_KRW 로 덮인다 = 환율이 움직이면 코드 손 안 대고 바꾼다.
USD_KRW = float(os.environ.get("USD_KRW") or "1500")
GEMINI_CALL_KRW = round(GEMINI_1K_USD * USD_KRW, 2)
FNAME = "cost.json"
