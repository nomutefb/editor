...
운영자 260811 확정 흐름:
  ① 운영자가 상황을 말한다(글 · 사진)
  ② **감독**(페이블 티어 / Opus 5.5 / GPT 6 Astra — 페이블 티어는 지금 Opus 5.5 대행)이 콘티를 짠다 = `sbmake.sh` 가 이미 하는 일
    ...
    for c in shots:
        ...
        for attempt in (1, 2):
            try:
                ...
                # ⚠ **기다리기 전에 접수부터 확인한다**(260813 실사고) — 번호를 돌려받았는데 그
                #   번호가 창구에 없는 일이 실재한다(회신이 글로 오면 번호 파서가 글 속 아무
                #   UUID 나 집을 수 있다). 구판은 그걸 모르고 상한까지 기다렸고(실측 50분)
                #   로그는 「큐가 밀렸다」고 말했다 = 증상과 원인이 정반대인 오진.
                #   접수가 안 됐으면 값이 안 나간 상태라 이 실패만은 **다시 쏴도 되는 축**이다.
