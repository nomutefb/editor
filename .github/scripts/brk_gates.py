...
③ 사법 절차 후속(judicial) — 구형·재판 시작·수사 착수·영장 신청/기각·입건·송치·기소·압수수색·소송·고소 = X.
   사형은 무조건 통과 · 구형은 무조건 X(선고 예고가 같이 있어도) · 선고·판결·무죄·유죄·법정구속·영장 발부는 통과 ·
   매체가 몰린 건(cross ≥ JUDICIAL_PASS_CROSS)은 전국적 사건으로 보고 통과.
...
JUDICIAL_PASS_CROSS = int(os.environ.get("BRK_GATE_JUDICIAL_CROSS", "8"))
...
# ── ③ 사법 절차 후속 ──────────────────────────────────────────────────────────────────────────
_JUD = re.compile(r"구형|재판 ?시작|첫 재판|첫 공판|공판|수사 ?착수|수사 ?본격화|수사 ?의뢰|경찰 ?수사|검찰 ?수사|입건|송치|불송치|재수사|"
                  r"영장|기소|항소심|2심 ?시작|상고|증거|재판행|고소|고발|소송|진정|압수수색|조사 ?착수|감사 ?착수|징계")
_JUD_KEEP = re.compile(r"무죄|유죄|선고|판결|법정 ?구속|영장 ?발부|확정|실형|배상")
...
def judicial_gate(title, cross=0):
    ...
    if "사형" in t:
        return None
    if (cross or 0) >= JUDICIAL_PASS_CROSS:
        return None
    if "구형" in t:
        return "사법 절차 후속(구형 = 검찰 요청 · 선고 아님)"
    if _JUD_KEEP.search(t):
        return None
    if _JUD.search(t):
        return "사법 절차 후속(수사·영장·기소·소송 단계 = 급발 아님)"
    return None
