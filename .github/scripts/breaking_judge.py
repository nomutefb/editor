...
⚠️ 또 **메이저급 연예인의 확정 결혼·결별·이혼·범죄 연루·구속**은 아래 «메이저급 연예인 혼인·사건 예외»로 O(그 외 연예인의 결혼·결별·이혼·열애설은 X).
...
② 컴백·신작·수상·화보·근황·완전체·콘서트 등 활동 소식 ③ 열애설 등 **미확정 관계 소식**(단 메이저급 연예인의 확정 결혼·결별·이혼은 아래 «메이저급 연예인 혼인·사건 예외»로 O)
④ 무명·중소 연예인, 인지도 불확실한 신인·인플루언서 ⑤ 스포츠 선수의 구단 이적(문화 콘텐츠 축 그대로 X).

⚠️ **메이저급 연예인 혼인·사건 예외 (운영자 260817 — «메이저 방송사·청룡영화제 이력 있는 연예인의 결별·결혼·범죄연루·구속은 긴급으로»):**
**메이저급 연예인**(아래 기준)의 **결혼·결별·이혼**과 **범죄 연루·피의자 입건·구속·기소**는 콘텐츠 소식이 아니라
...
⚠️ 다음은 이 예외로 열지 않는다(종전대로 X): ① **열애설·결별설·이혼설 등 미확정 관측·지라시**(공식 확인 전) — 결혼·결별·이혼은 **확정·공식 발표**분만
② 단순 열애 사실 공개(사귄다는 소식 자체)는 X — 결혼·결별·이혼 같은 **관계 확정/종료**만 O ③ 무명·중소·신인·인플루언서(위 수상 이력·전국 인지도 없음)
④ 컴백·신작·수상·화보·근황 등 활동 소식(종전대로 X).
