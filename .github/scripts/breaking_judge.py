...
- 사망·부상·인명피해·실종 등 인적 피해 — **단 아래 ⚖ 인명 문턱을 넘긴 것만**(재해·재난·사고 = 확정 사망 3명 이상 / 대인 강력범죄 = 피해자 2명 이상)
...
⚖ **인명 문턱 — 사람이 몇 명이어야 긴급인가 (운영자 260818 «살인 성폭행 등은 기준을 피해자 2명 이상 · 재해재난사고는 기준을 사망자 3명 이상 · 그럼 모녀가 숨져도 긴급은 아닌거지»):**
사람 수를 근거로 O를 낼 때는 **축마다 숫자가 다르다**. 이 문턱은 아래 개별 게이트보다 **먼저** 적용한다.
- **재해·재난·사고 축**(자연재해·화재·산불·폭발·붕괴·추락·침몰·중독·교통사고 등) = **확정 사망 3명 이상**이어야 O. 2명 이하는 X.
  ⚠️ **확정 사망만 센다** — 실종·매몰·고립·부상·이송·심정지·의식불명·구조·대피는 이 숫자에 넣지 않는다.
...
- **일가족·가족·일가·이웃 주민** 등 인원이 안 드러나는 말 = 수 미상 → 재해·재난·사고 축에선 **문턱 미충족으로 본다**(숫자가 같이 적혀 있으면 그 숫자를 쓴다 — 「일가족 3명 숨져」 = 3명 = O).
- 숫자와 관계어가 같이 있으면 **숫자가 우선**(「일가족 4명 숨져」 = 4명).
⚠️ 이 문턱은 **사람 수를 근거로 판정할 때만** 쓴다 — 사망자가 제목에 안 적힌 채 대피·구조·매몰·고립 같은 **인명 위협**만 있는 건은
아래 각 게이트의 종전 기준 그대로 판정한다(그 축은 사망자 수 축이 아니다).
...
⚠️ 단 **사고·화재·재난·붕괴·중독 등 *사고성***은 이 게이트 미적용 — 그쪽은 ⚖의 **확정 사망 3명** 문턱과 각 게이트로 판정한다.
...
⚠️ **관계어는 ⚖ 환산표대로 센다**(모자·모녀·부부 = 2명 = 문턱 미달 X · 「일가족 3명」처럼 숫자가 적히면 그 숫자).
...
⚠️⚠️ **사망자가 적힌 화재는 ⚖ 문턱을 먼저 통과해야 한다(운영자 260818) — 확정 사망 3명 이상만 O, 2명 이하는 X.**
「모자 숨져」·「모녀 숨져」는 2명(⚖ 관계어 환산)이라 **X**다. 사망자 수가 안 적힌 채 **대피·구조·매몰·고립** 등 인명 위협만 있는 건은
그 축이 사망자 수 축이 아니므로 **종전 그대로 판정**한다(대규모 대피·다수 매몰 = O 유지).
⚠️ 이 게이트는 **화재·산불 축 전용** — 폭발·붕괴·추락·침몰 등 다른 사고성은 종전 일반 규칙 + ⚖ 확정 사망 3명 문턱으로 판정한다.
  ...
  예) "아파트서 불…90대·60대 모녀 숨져"(확정 사망 2명) → **X** · "화재로 모자 숨진 아파트 세대, 과거 «분말소화기 미비치» 지적받아"(2명) → **X**
  예) "아파트 화재로 일가족 4명 숨져"(4명) → O · "공장 화재로 3명 매몰"(사망 미명시·다수 매몰) → O · "산불 확산에 주민 200명 대피"(사망 미명시·대규모 대피) → O
...
- **재해·재난·사고인데 확정 사망이 2명 이하**인 것 — 화재·폭발·붕괴·추락·침몰·중독·교통사고 포함(⚖ 문턱 · 「모자·모녀 숨져」 = 2명 = 여기)
