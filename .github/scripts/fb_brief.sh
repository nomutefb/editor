...
[존재 이유] 기간 창끼리 추이를 비교해 전략 방향을 잡는 도구다 — 창이 넓어질수록 같은 데이터가 달리 읽힌다. 짧은 창의 '문제'가 긴 창에선 '정상 리듬'인 지점을 짚어라. 매 섹션 마지막 줄 = 그 창에서만 보이는 전략 시사점 한 줄. ⚠ 운영자는 원자료를 매일 직접 본다(운영자 260827) — 데이터 재낭독은 정보 증가 0, 「그래서 지금 뭘 하나」를 골라라.
...
[전체]
[총론]
- [3일] = **지금 이 순간 — 사흘의 일지가 아니다**(운영자 260827 「3일치는 매일 보는 거니 3일을 아울러 오늘 이 순간에 대한 조언」). 여는 인사 반 줄 → 첫 문장 = **지금 움직이는 것 + 오늘 첫 수**(결론부터·두괄식) → 근거 실측 1~3줄(사흘에서 결론을 떠받치는 것만 · 날짜별 일지 금지 · 결론에 안 걸리는 지표는 안 쓴다) → 사흘이 7일·28일과 같은 방향인지 한 줄. 3~5줄.
