...
MODEL = os.environ.get("FABLE_MODEL") or "claude-opus-5-5"    # 프롬프트 작성 = 페이블 티어(운영자 260721 "AI 이미지 생성은 품질이 많이 차이나니까 FABLE 5" — 그 전엔 오퍼스) · 값 = models.json tiers.fable(대행·복귀는 apply_models.py가 키로 옮김) · env 오버라이드 유지
...
EFFORT = os.environ.get("PIPE_EFFORT", "high")            # 연료 방어(운영자 260721 "낭비 새는 구간 없게"): Fable 5 max = 과사고·장시간 낭비 위험 — high도 구 Opus max 이상 품질(모델 카드 정본) · 구 "opus 5.0 --effort max"의 Fable 등가
