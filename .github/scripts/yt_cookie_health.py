...
def main():
    ...
    if ok is True:
        ...
        if n >= DEAD_MIN:
            # 건수 접미 회전(고정 id = 한 번 열면 영구 실명 · brk_misfire 교훈)
            # ⚠⚠ 260817 실측 봉합 = **회차 접미만으로는 번호가 재사용된다.** 살아나면 dead_streak 가 0으로
            #   리셋되므로 다음 사망 구간이 다시 2·3·4… 를 쓴다. 그런데 운영자가 지난 구간에 그 번호를
            #   「치웠다」면(계정 md 축) 뷰어 `msgRptRows` 가 그 id 를 **필터로 제거**해 그 회차 경고가
            #   화면에서 통째로 사라진다 — 실측 = 계정 md 에 `yt-cookie-dead-3` 이 있어(오늘 치운 그 알림)
            #   다음 사망 3회차가 침묵한다. 비용이 큰 이유 = 「2회차 경고를 보고 갈았는데 또 죽었다」가
            #   정확히 그 구간이라, 운영자가 「고쳐졌구나」로 읽는다(경고가 안 오는 것이 아니라 **꺼진 것**).
            #   이 파일 머리 설계원칙 ②가 「고정 id = 영영 재점등 안 됨」을 이미 경고했는데, 회전 축을 회차로
            #   잡아 리셋 뒤 같은 병이 되살아나는 구조였다 = 회피가 전제를 잃은 자리.
            # → 사망 **구간 도장**을 붙인다: 구간 = 마지막 정상 시각(그 구간 내내 불변 · 살아나면 갱신되니
            #   다음 구간은 다른 도장 = 재사용이 구조적으로 불가). 회차 접미는 유지(계속 죽으면 계속 새 알림).
            #   문법 정본 = 이 레포 관례인 `sys:quake:`+time 회전(설계원칙 ②가 지목한 그 축).
            #   ⚠ 년까지 넣는다 — 월일시분만 쓰면 1년 뒤 같은 도장이 나와 같은 병이 되살아난다.
            tag = re.sub(r"\D", "", meta.get("last_ok") or now) or "0"
            mid = f"{MSG_ID_BASE}-{tag}-{n}"
