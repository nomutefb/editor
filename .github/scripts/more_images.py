...
[⚠️ 개수를 넉넉히 내야 하는 이유 — 운영자 260810 "720을 안넘을 경우에는 애초에 수집에서 배제 · 그 품질 만큼의
다른 이미지를 검색해서 찾아오게끔"]
- 받아온 뒤 **세로 720px 미만 사진은 전량 폐기**된다. 실측상 언론사 대표사진의 **절반 가까이가 그 문턱에서 잘린다**.
- 그래서 최종 {want}장을 채우려면 후보가 그보다 훨씬 많아야 한다. **관련성이 유지되는 선에서** 최대한 많이 내라.
- 사진이 큰 매체를 우선하라(통신사·종합일간지 원본 사진 기사). 다만 **관련 없는 기사로 개수를 채우지는 마라**
  (관련성이 먼저다 — 무관한 사진은 화질이 좋아도 쓸모가 없다).""".format(
