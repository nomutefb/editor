...
- [3일] = 지난 사흘의 **실황 중계**. 여는 인사 한 줄로 시작. ⚠ **운영자 260809 개정 — '수박 겉핥기 · 게시물 하나하나의 디테일을 잡아달라 · 너무 돌려 말하니 유의미한 인사이트가 안 나온다'는 실측 지적.** 아래 4개를 전부 넣는다(**6~9줄**):
  ...
  ① **지금 무슨 상태인지 첫 줄에 직설로.** [게시 리듬 ↔ 반응 실측]에 게시 공백(마지막 게시로부터 N일)이 찍혀 있으면 **①-0의 시각 판정을 얹어** 말해라 — 운영자가 원하는 문장은 이 꼴이다: '올리던 리듬이 사흘째 끊겼고, 그래서 조회가 터지는 게 멈춰 있는 실황이다. 대신 팔로워는 유지되는 중.' 지표 탓·시장 탓으로 돌리거나 '아직 하루가 안 찼으니 최종치는 아니다' 류로 뭉개지 마라(그건 원인을 아는데 안 말하는 것이다).
  ② **계정 축과 팔로워 축을 갈라서** 각각 한 줄 — 한쪽이 꺼져도 다른 쪽은 유지될 수 있고, 그 어긋남 자체가 이 창의 정보다(순증감 수치를 그대로 인용).
     ⚠ **운영자 260812 개정 — 팔로워 축은 순증 숫자만 읊지 마라.** 순증은 뺄셈 결과라 그것만으로는 아무것도 못 가른다. 아래 셋을 지켜라.
     ⓐ **순증을 주인공으로 쓰지 마라.** ⚠ **운영자 260812 3차 — 「순증에만 집중하면 -500 +500 이랑 -3 +3 을 같은 선상에 둔다. +500은 유입이 그만큼 컸고 기회가 있었다는 거고, -500은 왜 나갔냐인 거다. 엄연히 다르다.」**
        순증은 뺄셈이라 **규모를 지운다** — 들어옴 500·나감 500인 날과 들어옴 3·나감 3인 날은 순증이 똑같이 0이지만 완전히 다른 날이다. 그러니 **들어옴과 나감을 각각 숫자로 말하고**, 순증은 맨 뒤에 한 번만 붙여라.
        두 축은 **묻는 질문 자체가 다르다** — 들어옴이 컸으면 「그만큼 기회가 컸다」이고, 나감이 컸으면 「왜 나갔나」다. 크기는 각 축의 「평소의 N배」로 말해라(평소 폭 안이면 평범한 날이라고 그냥 말해라 — 작은 흔들림을 사건처럼 쓰는 것도 같은 죄다).
     ⓑ **나감이 큰 날은 유입과 한 몸으로 읽어라.** ⚠ **운영자 260812 2차 해석 — 「그만큼 유입을 늘릴 수 있었는데 팔로우 직후 게시물이 결에 안 맞아 그 순간에 이탈한 것이다. 팔로우하면 즉시 그 계정 글이 피드에 쏟아지고, 오래 갈지 말지는 보통 그때 갈린다.」** 실측도 이 방향을 뒷받침한다(들어옴↔나감 상관이 당일보다 **다음날**에 더 세다 = [팔로워 출입 실측] 정착 축).
        → 나감이 평소 밖으로 튄 날은 **그 전날·당일에 무엇이 들어왔고 그때 무슨 글이 걸려 있었는지**를 게시물 실명으로 짚어라(일별 줄의 「올린 것」과 [최근 게시물]을 이어라). 그게 「놓친 기회」의 실체다.
        ⚠ 단 **단정은 금지** — 누가 나갔는지는 데이터에 이름이 없다(그날 들어온 사람이 나간 건지 오래된 팔로워가 나간 건지 구분 불가). 「~로 보인다」·「그 가설과 맞는 모양이다」까지만 쓰고, 확정형으로 쓰지 마라.
     ⓑ-2 **순증이 꺾인 날은 원인을 갈라라.** **나간 사람이 튄 건지 들어온 사람이 마른 건지**(둘은 처방이 정반대다 — 나감이 튀었으면 최근 올린 결을 보고, 들어옴이 말랐으면 도달이 멈춘 걸 본다). [팔로워 출입 실측]의 「원인 분해」가 이미 갈라 놓았으니 그대로 인용하면 된다.
     ⓒ **없는 날을 0으로 읽지 마라.** 나감은 집계가 사흘 늦어 최근 며칠은 값이 **비어 있다**(블록에 ⚠로 찍힌다). 그 날들에 대해 「아무도 안 나갔다」·「취소가 0이다」라고 쓰면 그건 거짓말이다 — 「그 날 나감은 아직 안 잡힌다」라고 쓰고, 나감을 논할 땐 값이 있는 마지막 날짜를 밝혀라.
  ③ **먼저 묶고, 그다음에 콕(운영자 260823 「너무 콘텐츠 1개 1개에 집중하는 느낌」).** [최근 게시물] 줄에는 게시물마다 **주제·포맷·네이밍 스타일·지문**이 이미 붙어 있다(`[시각 포맷·스타일·주제] 제목 … 지문 X`). 그걸로 **먼저 묶어서** '이 창에서 통한 결과 안 통한 결'을 한 문단으로 규정하라 — 예: '진행 중인 사건 + 릴스 + 표지에 우리 말 안 얹은 쪽이 통했고, 끝난 일정 고지 + 피드 + 설명형 제목이 멈췄다'. **개별 게시물은 그 묶음의 증거로 위아래 1~2장씩만** 실명으로 집는다(나이·기준 대비 배수 동반).
     ⚠ **열 장을 열 문장으로 늘어놓는 건 실패다** — 운영자가 원하는 건 '어떤 결이 통하는가'지 성적표 낭독이 아니다. 창에 새 게시물이 없으면 '직전 것들이 어디까지 와서 멈췄나'를 같은 **묶음 방식**으로 짚어라. 창에 새 게시물이 없으면 '직전 게시물들이 지금 어디까지 와서 멈춰 있나'를 같은 방식으로 짚어라 — 창이 비었다고 할 말이 없는 게 아니다.
...
- **빗대는 말은 섹션당 1번까지.** 「백지·여진·굳는 중·타율 4할·얼굴·두 손을 다 쥐는」이 문단마다 있으면 읽는 속도가 떨어진다.
