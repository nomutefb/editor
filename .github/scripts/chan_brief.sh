...
. shared/tone_block.sh          # 한국어 결 공용 블록(TONE_BLOCK) SSOT — 운영자 260823 «sns 요약에도 그 말투» · v19 말투 3안이 우선
...
if series:
    ...
    L.append('  ※ «…» 안은 **카드 표지에 실제로 박힌 제목**(260812~ · ` || ` = 표지에서의 줄바꿈)이다. 표지 판독 전 게시물만 글 첫 줄이 쓰인다.')
    for r in series[-30:]:
        # 게시물 이름 = **표지에 박힌 제목**(ovt) 1순위 · 없으면 글 첫 줄(name) — 운영자 260812
        # "기사 인트로 첫줄보다, 오버레이가 가장 정확한 내용이거든". 노뮤트는 카드 제목과 글 첫 줄을 각각
        # 따로 쓰기 때문에(실측 = 「🚨 엿새 만에 다시 쐈다…」 ↔ 카드 「북한, 엿새 만에 '또 쐈다'」) 첫 줄로
        # 지목하면 요약이 화면에 뜬 적 없는 문장을 게시물 이름이라고 부른다. 판독 = insta_cover_ocr.py.
        # ⚠ 22자 컷은 첫 줄 기준이라 표지 제목(머리줄+받침줄)을 반토막 낸다 → 이름 축만 60자로 넓힌다.
        _rf = ' / '.join(f"«{(str(x.get('ovt') or '') or str(x.get('name') or '(무캡션)'))[:60]}» {fv(x.get('views'))} 링크 {x.get('permalink') or '—'}" for x in (r.get('post_refs') or [])[:3])
        _nt = r.get('follower_net')
        # 들어옴/나감 분해(운영자 260812) — 순증 한 칸만 싣던 자리. 나감은 집계 3일 지연이라 최근 3일은 '—'.
        _fi, _fo = r.get('follows'), r.get('unfollows')
        _io = ''
        if isinstance(_fi, int) or isinstance(_fo, int):
            _io = (f"(들어옴 {format(_fi, ',') if isinstance(_fi, int) else '—'}"
                   f" · 나감 {format(_fo, ',') if isinstance(_fo, int) else '—'})")
        L.append(f"{str(r.get('date',''))[5:]} 조회 {fv(r.get('views')) if r.get('views') is not None else '—'}"
                 ...
                 f" · 팔로워 {(('+' if _nt > 0 else '') + format(_nt, ',')) if isinstance(_nt, int) else '—'}{_io}"
                 + (f" · 올린 것: {_rf}" if _rf else ''))
    # ── [팔로워 출입 실측] 신설(운영자 260812 "-96 빠졌는데 · 취소 기준으로 보면 많이 빠진 거 · 이런 데이터가 요약에 반영되어야")
    # ⚠ 진범 = 요약이 읽는 칸에 **뺄셈 결과 한 칸**만 있었다. 들어옴·나감 실측은 179일치 원장으로 이미 들어와 있는데
    #   (insta_fetch follows_and_unfollows → daily_series follows/unfollows) 프롬프트에 한 번도 안 실렸다
    #   → 모델은 「+24 · -9」만 말할 수 있었고 「몇 명이 나갔나」는 **원리적으로 말할 수 없었다**.
    #   그래서 -9와 -96이 같은 말투로 취급된다(운영자 지적 = 「-3 +3 하고 -96 +96은 많이 차이나는 건데」).
    # ⚠ 두 번째 사각 = **크기 감각**. 순증을 그냥 나열하면 모델이 그날이 평소 폭 안인지 밖인지 셀 근거가 없다
    #   → 평소 흔들림(중앙절대편차)을 같이 실어 「평소 밖 N배」를 기계가 준다.
    #   ⚠ 표준편차 대신 중앙절대편차를 쓴다 = 실측 순증 SD 134.5는 8/2 +504 같은 폭발일 한 건이 부풀린 값이라
    #     그걸 기준자로 쓰면 어지간한 마이너스가 전부 「평소 범위」로 삼켜진다(가짜 초록).
    # ⚠ 나감 집계는 3일 지연(insta_fetch FOLLOW_LAG) — 최근 3일은 값이 **없다**. 이걸 블록에 못박는다.
    #   침묵하면 모델이 결측을 0으로 읽고 「어제는 아무도 안 나갔다」는 없는 결론을 만든다(이 레포 반복 병).
    try:
        import statistics as _sti
        _nn = [r['follower_net'] for r in series[-30:] if isinstance(r.get('follower_net'), int)]
        _oo = [(r['date'], r['unfollows'], r.get('follows')) for r in series[-30:]
               if isinstance(r.get('unfollows'), int)]
        if len(_nn) >= 5:
            _nmd = round(_sti.median(_nn))
            _mad = round(_sti.median([abs(x - _nmd) for x in _nn])) or 1
            def _dev(x): return round(abs(x - _nmd) / _mad, 1)
            # ⚠ 「빠진 날」은 문턱과 무관하게 **전건** 싣는다 — 운영자가 실제로 보는 축이 이쪽인데(260812 지적),
            #   문턱만 걸면 위로 터진 날(+563 같은)이 목록을 다 먹고 마이너스 날이 밀려난다(실측 첫 판이 그랬다).
            _mn = [r for r in series[-30:] if isinstance(r.get('follower_net'), int) and r['follower_net'] < 0]
            _up = sorted((r for r in series[-30:] if isinstance(r.get('follower_net'), int)
                          and r['follower_net'] > 0 and _dev(r['follower_net']) >= 2),
                         key=lambda r: -r['follower_net'])
            _lo3 = (('줄어든 날(전건): ' + ' · '.join(f"{r['date'][5:]} {r['follower_net']}(평소 밖 {_dev(r['follower_net'])}배)"
                                                  for r in _mn)) if _mn else '줄어든 날 없음')
            if _up:
                _lo3 += (f" / 위로 튄 날 {len(_up)}일(최대 "
                         + ' · '.join(f"{r['date'][5:]} +{r['follower_net']}({_dev(r['follower_net'])}배)" for r in _up[:3]) + ')')
            _ul = ''
            if len(_oo) >= 5:
                _uv = [u for _, u, _f in _oo]
                _umd = _sti.median(_uv)
                _usd = round(_sti.pstdev(_uv), 1)
                _fv2 = [f for _, _u, f in _oo if isinstance(f, int)]
                _fmd = _sti.median(_fv2) if _fv2 else None
                _gap = sum(1 for r in series[-3:] if isinstance(r.get('follower_net'), int)
                           and not isinstance(r.get('unfollows'), int))
                # 마이너스·저조일의 원인 분해 = 나감이 튀었나 vs 들어옴이 말랐나(둘은 처방이 정반대다)
                _why = []
                for dt, u, f in _oo[-14:]:
                    if not isinstance(f, int):
                        continue
                    if u > _umd + 2 * (_usd or 1):
                        _why.append(f"{dt[5:]} 나감 {u}(평소 {_umd}) = **나간 사람이 튄 날**")
                    elif _fmd and f <= _fmd * 0.6:
                        _why.append(f"{dt[5:]} 들어옴 {f}(평소 {_fmd}) · 나감 {u}(평소 수준) = **들어온 사람이 마른 날**")
                # 축 1 = 들어옴(그날 기회가 얼마였나) · 축 2 = 나감(왜 나갔나) — 각각 규모와 튄 날을 따로 낸다.
                _fall = [r['follows'] for r in series[-30:] if isinstance(r.get('follows'), int)]
                _fbig = ' · '.join(
                    f"{r['date'][5:]} {r['follows']}(평소의 {round(r['follows'] / _fmd, 1)}배)"
                    for r in sorted((r for r in series[-30:] if isinstance(r.get('follows'), int)
                                     and _fmd and r['follows'] >= _fmd * 1.5),
                                    key=lambda r: -r['follows'])[:4]) if _fmd else ''
                _ubig = ' · '.join(
                    f"{r['date'][5:]} {r['unfollows']}(평소의 {round(r['unfollows'] / _umd, 1)}배)"
                    for r in sorted((r for r in series[-30:] if isinstance(r.get('unfollows'), int)
                                     and r['unfollows'] >= _umd * 1.2),
                                    key=lambda r: -r['unfollows'])[:4])
                _ul = (f" || **들어옴 축 = 「기회가 얼마였나」**: 중앙 {_fmd if _fmd else '—'}명/일"
                       + (f" · 폭 {min(_fall)}~{max(_fall)}" if _fall else '')
                       + (f" || 크게 튄 날(= 그만큼 기회가 컸던 날): {_fbig}" if _fbig else ' || 평소 밖으로 튄 날 없음')
                       + f" || **나감 축 = 「왜 나갔나」**: 중앙 {_umd}명/일 · 폭 {min(_uv)}~{max(_uv)} · 표준편차 {_usd}"
                       + (' = 거의 일정' if _usd < _umd * 0.3 else '')
                       + (f" || 크게 튄 날(= 이유를 찾아야 하는 날): {_ubig}" if _ubig else ' || 평소 밖으로 튄 날 없음')
                       + (f" · ⚠최근 {_gap}일은 나감 미수집(집계 3일 지연 = 결측이지 0이 아니다)" if _gap else '')
                       + (f" || 순증이 꺾인 날 원인 분해: {' · '.join(_why[-5:])}" if _why else ''))
                # ── 정착 축(운영자 260812 2차 해석) = 「들어온 사람이 그 자리에서 붙었나」
                # 운영자 관점 = 「-98은 유입을 그만큼 늘릴 수 있었는데 놓친 것이다. 팔로우하는 즉시 그 계정 글이
                #   피드에 쏟아지고, 오래 갈지 말지가 보통 **그때** 갈린다」 → 취소는 유입과 **한 몸으로** 읽어야 한다.
                # ⚠ 실측이 이 가설을 뒷받침한다(원장 179일 · 이 세션 실측) = 들어옴 ↔ 나감 상관이
                #   당일 0.475 → **다음날 0.572(최대)** → 이틀 뒤 0.405로 **다음날에 봉우리**를 만든다.
                #   유입 상위 8일 전건에서 다음날 나감이 평소의 2~3.7배로 뛴다(4/20 = 유입 2071 → 다음날 238).
                # ⚠⚠ **그래도 단정은 못 한다** — 누가 나갔는지는 API가 이름을 안 준다(코호트 추적 원리적 불가).
                #   상관은 「그날 들어온 사람이 바로 나갔다」와 **양립**할 뿐 증명이 아니다(오래된 팔로워가
                #   같은 글을 보고 나간 것일 수도 있다). 그래서 블록은 **배수만 사실로** 내고 판정은 모델·사람 몫.
                # ⚠ 「기저를 빼서 초과 이탈을 낸다」는 첫 판은 **폐기**했다 = 기저를 전 구간(179일)에서 뽑으면 74명인데
                #   최근 30일 나감 중앙은 65라 **최근 전건이 초과 0으로 뭉개진다**(실측 = 가짜 초록 공장 · 5~6월만 빨갛다).
                #   계정이 크면 기저도 같이 움직여서 한 값으로 못 잡는다 → **평소 대비 배수**가 왜곡이 적다.
                try:
                    _ux = {r['date']: r['unfollows'] for r in series if isinstance(r.get('unfollows'), int)}
                    _fx = {r['date']: r['follows'] for r in series if isinstance(r.get('follows'), int)}
                    import datetime as _dtn
                    def _nx(k): return (_dtn.date.fromisoformat(k) + _dtn.timedelta(days=1)).isoformat()
                    # ⚠ 이 축만 **전 기간**을 쓴다 — 최근 30일엔 유입 폭발일이 거의 없어 표본이 안 선다.
                    #   창이 다르면 밝힌다([1] 정직) · 다음날 나감이 원장에 없는 날은 **아예 뺀다**
                    #   (첫 판이 「아직 미수집」이라 썼는데 1월 건은 미수집이 아니라 원장 보관 시작 전이라 거짓 문구였다).
                    _big = [k for k in sorted((k for k in _fx if _fmd and _fx[k] >= _fmd * 1.5),
                                              key=lambda k: -_fx[k]) if _nx(k) in _ux][:4]
                    _st = ' · '.join(
                        f"{k} 들어옴 {_fx[k]}(평소 {_fmd}) → 다음날 나감 {_ux[_nx(k)]}"
                        f"(평소 {_umd}의 **{round(_ux[_nx(k)] / _umd, 1)}배**)"
                        for k in _big)
                    # 나감이 평소 밖으로 튄 날 = 그 **전날·당일 유입**을 나란히(들어온 사람이 나간 건지 볼 재료)
                    _sp = ' · '.join(
                        f"{k} 나감 {_ux[k]}(평소의 **{round(_ux[k] / _umd, 1)}배**) ← 전날 들어옴 "
                        + str(_fx.get((_dtn.date.fromisoformat(k) - _dtn.timedelta(days=1)).isoformat(), '—'))
                        + f" · 당일 들어옴 {_fx.get(k, '—')}"
                        for k in sorted(_ux, key=lambda k: -_ux[k])[:3] if _ux[k] >= _umd * 1.2)
                    if _st or _sp:
                        _ul += (f" || **정착 축**(= 들어온 사람이 그 자리에서 붙었나 · ⚠창 = **전 기간 {len(_ux)}일**"
                                "[최근 30일엔 유입 폭발일이 없어 표본이 안 선다] · 실측: 유입↔나감 상관이 당일보다"
                                " **다음날**에 더 세다 = 팔로우 직후 피드에 그 계정 글이 쏟아지는 그 순간이 갈림길)"
                                + (f" || 유입이 크게 튄 날의 다음날: {_st}" if _st else '')
                                + (f" || 나감이 크게 튄 날: {_sp}" if _sp else '')
                                + " || ⚠ 누가 나갔는지는 데이터에 이름이 없다 = 「그날 들어온 사람이 나갔다」는 **단정 금지**"
                                  "(배수는 그 가설과 양립할 뿐 증명이 아니다)")
                except Exception:
                    pass
            # ⚠⚠ 순서가 계약이다(운영자 260812 3차) — 「순증에만 집중하면 -500+500 이랑 -3+3 을 같은 선상에 둔다.
            #   +500은 유입이 그만큼 컸고 **기회가 있었다**는 거고, -500은 **왜 나갔냐**인 거다. 엄연히 다르다.」
            #   → 순증은 뺄셈이라 **규모를 지운다**. 그래서 들어옴·나감을 **각각 앞에** 세우고 순증은 참고로 내린다.
            #   두 축은 묻는 질문이 다르다 = 들어옴 = 「기회가 얼마였나」 / 나감 = 「왜 나갔나」.
            L.append(f"[팔로워 출입 실측(최근 30일)] ⚠ **순증은 뺄셈이라 규모를 지운다** — 들어옴 500·나감 500인 날과"
                     f" 들어옴 3·나감 3인 날은 순증이 똑같이 0이지만 완전히 다른 날이다(앞은 기회가 500이었고, 뒤는 아무 일도 없었다)."
                     f" 아래 두 축을 **각각** 읽어라." + _ul
                     + f" || (참고) 순증 중앙 {('+' if _nmd > 0 else '')}{_nmd}명/일 · 평소 흔들림 ±{_mad}"
                       f"(중앙절대편차 = 폭발일 1건에 자가 늘어나는 것 차단) · {_lo3}")
    except Exception:
        pass
...
body = '\n'.join(L)
PVER = 'chanbrief-v21-260827-now' + '-t' + __import__('os').environ.get('TONE_VER', '')   # 260908: 한국어 결 정본 해시 결합 = 규칙 개정 시 자동 캐시 버스트(감사 R4)   # v21 = 오늘 중심·간결(운영자 260827 「3일치는 매일 보는 거니 3일을 아울러 오늘 이 순간에 대한 조언 · 알 수 없는 지표 얘기와 부수적 표현이 너무 많다 · 간결하고 읽기 쉽게」) — [3일] = 결론부터(두괄식 · 오늘 첫 수 선행) + 근거는 결론 떠받치는 실측 2~4줄만 + 결측 지표 서사 금지(빼고 한 마디) + 감상·자평·빗대기 컷(판 전체 1번) + 배수 기준 명시 · FB v9 동반   # 구   # v20 =한국어 결 공용 블록 편입(운영자 260823 «sns 요약에도 그 말투» · 정본 shared/tone_block.sh · v19 말투 3안 위에 문장 무늬 축만 추가 = 레인 톤 우선) · 구   # v19 = 한국말 자연스럽게(운영자 260823 「말이 어색하고 이해하기 어려워」) — 말투 3안 채택(팬픽·웹소설 문체 폐지·압축 해제·아라비아 숫자 강제) + 시간 호흡(아침에 게시 공백을 지적으로 쓰지 않는다) + 먼저 묶고 그다음 콕(개별 나열 → 결 단위 규정) + 왜 터졌나를 결로 답하기   # 구   # v18 = 규모 축 우선(운영자 260812 3차 「순증에만 집중하면 -500+500 이랑 -3+3 을 같은 선상에 둔다 · +500은 기회가 컸다는 거고 -500은 왜 나갔냐인 거다」) — 들어옴·나감을 각각 앞에 세우고 순증은 참고로 내림 + 정착 축(운영자 2차 해석 「팔로우 직후 피드에 글이 쏟아지는 그 순간이 갈림길」 · 실측 유입↔나감 상관이 다음날에 더 셈) + 나감 튄 날을 전날·당일 유입과 나란히   # 구   # v17 = 팔로워 들어옴/나감 분해(운영자 260812 "-96 빠졌는데 · 취소 기준으로 보면 많이 빠진 거 · 이런 데이터가 요약에 반영되어야") — 179일치 실측이 원장에 있는데 프롬프트엔 뺄셈 결과 한 칸뿐이라 「몇 명이 나갔나」를 원리적으로 말할 수 없었다: 일별 줄 (들어옴·나감) + [팔로워 출입 실측] 블록(평소 흔들림 = 중앙절대편차 · 꺾인 날 원인 분해 · 나감 3일 지연 결측 명시) + [3일]② 3계약(크기·원인·결측)   # 구   # v16 = 테마 이탈   # v15 = 게시물 링크 참조(골드·이탤릭 .tbrief-ref 계승) + 강조 밀도 개정(2층 섹션당 3~5·1층 1) + [3개월] 기 대 기 리듬(운영자 260809 3차)   # 구   # v14 = 7일·28일 확장   # v14 = [7일]·[28일]에 [3일] 실황 문법 확장(운영자 260809 2차 "배선 ㄱㄱ") — 7일 = 그 주 게시물 한 장씩 개별 판정+축 분리 · 28일 = [주 단위 리듬] 4주 블록 신설(중앙 초과 장수 = 그 주 성패 축 · 구판은 모델이 30줄을 손으로 세야 했다)   # 구   # v13 = [3일] 실황   # v13 = [3일] 실황 두껍게(운영자 260809 "수박 겉핥기 · 게시물 하나하나 디테일 · 너무 돌려 말한다") — 실측 진범 = 데이터 공백: views 최근 30일 전건 결측 + follows 말미 0-fill이라 모델이 사흘을 말할 일별 근거가 0칸이었다 → 살아있는 축(도달·팔로워 순증감·그날 올린 것 실명) 편입 + [게시 리듬 ↔ 반응] 신설 + 게시물 나이·중앙 대비 배수 + 완곡어법 금지 전 섹션 계약   # 구   # v12 = 총론 디테일 복원(운영자 260808 5차 실측 지적 — 게시물 실명·이유·바깥 시류·알고리즘 추정·흐름 제안 3문단 필수 · ⑦ 시류 원료 동반)   # 구   # v11/v5 = ⑥ 정체 축 → 오늘의 행동 1개 강제(운영자 260808 3차 — 반복만 하고 안 옮겨지던 고리 절단)   # 구   # v10 = 누적 지식 라이브러리(운영자 260808 — 직전 1회차 1500자[전문의 36%]만 보던 것을 24회차 판단 원장으로 교체: 정체성 궤적·총론 방향·반복 제안·갈린 축 · 정본 apps/insta/brief_lib.py)   # v9.4 = 팔로워 표본 기준일 딱지(운영자 260726 · 밀리면 ⚠N일 전 병기) · v9.3 = 총론 '→ 그래서 무엇을' 결론 1줄 필수 + 강조 밀도 상향(운영자 260717 — 총론이 최장 판인데 볼드 2개·강조색 0으로 밋밋 실측 → 볼드 넉넉히·핵심어 1층)   # v9.2 = 신뢰 게이트(운영자 260715 Q06 — 거의 확실만 해석·방향)   # v9.1 = 알고리즘 협착 가설+실측(운영자 260715 Q05) · v9 = 회초리·표본(운영자 260715 Q02·Q03 — 인과 실측·팔로워 표본·반응 지문·확장문)   # 프롬프트 버전 — 바뀌면 해시 불일치 = 다음 run 강제 재생성 · v8 = 총론 분리(운영자 260714 "총론=비전·방향성·미션 큰 그림 3~12개월 / 전체=전체 기간 분석 디테일" — [전체 총론] 1부 → [전체]+[총론] 2부 = 6부) · v7 = 강조 2층 · v6 = 시간대·요일교란 · v5 = 존재이유+연재+아카이브 · v4 = 3일신설 · v3 = 5부 · v2 = 프리앰블금지
...
[존재 이유 — 운영자가 이걸 읽는 목적]
이슈 터졌나 감시하는 게 아니다. *기간 창끼리 추이를 비교해 전략 방향을 잡는* 도구다 — 창이 넓어질수록 같은 데이터가 달리 읽히는 지점이 핵심. 짧은 창에서 '문제'로 보이던 게 긴 창에선 '정상 리듬'이거나 그 반대인 지점을 명시적으로 짚어라. 매 섹션의 마지막 줄 = 그 창에서만 보이는 전략적 시사점 한 줄(이 창을 열어보는 이유가 되는 문장). ⚠ **운영자는 원자료를 매일 직접 본다(운영자 260827)** — 데이터를 다시 읽어주는 건 정보 증가 0이다. 네 값은 데이터에서 「그래서 지금 뭘 하나」를 골라주는 데 있다.
...
[전체]
[총론]
- [3일] = **지금 이 순간의 실황 중계 — 사흘의 일지가 아니다.** ⚠ **운영자 260827 개정 — 「3일치 데이터는 내가 매일 직접 본다. 3일에 대한 얘기보다는, 3일을 아울러서 '오늘' 바로 지금 이 순간에 대한 조언을 달라」**(구 「지난 사흘의 실황」 계약이 날짜별 나열로 흘렀다 · 260809 「겉핥기 금지」 취지는 ②의 실측 밀도로 계승 = 짧다≠얇다: 얇음은 근거 없는 뭉뚱그림이고, 간결은 근거만 남기고 수사를 지운 것). 사흘 데이터는 재료일 뿐 주제는 언제나 **오늘**이다 — 날짜별로 하루씩 훑어 내려가는 일지가 나오면 이 섹션은 실패다. 아래 ①~④ 순서대로(**5~8줄**):
  ⓪ **[단위 계약 — 어기면 그 문장은 통째로 거짓말이다]** 이 데이터에는 서로 다른 두 자가 있다.
     ...
     · **나감(취소)** = 그날 팔로우를 끊은 사람 수. 일별 줄 괄호의 「나감 74」·[팔로워 출입 실측]의 그 자다.
  ...
  ①-0 **지금 몇 시인지부터 계산하고 시작한다(운영자 260823 「나의 시간 호흡도 고려가 되었으면」).** 이 프롬프트 머리에 「지금 시각(한국)」이 실려 있다 — 그걸 읽고 운영자가 하루 중 **어디에 서 있는지**를 먼저 잡아라. 새벽·아침(~10시) = 자고 일어나 **이제 하루가 시작되는 시점** · 낮(10~18시) = 하루가 도는 중 · 저녁·밤 = 하루가 닫히는 시점.
     ⚠ **아침에 「오늘 아직 안 올렸다」를 지적처럼 쓰지 마라 — 취지가 어긋난다.** 자고 일어난 시점에 그건 당연한 상태다(운영자 260823 「kst 기준 아침이면 당연히 자고 일어난 시점인데 안올렸더라, 이게 취지가 안맞거든」). 그 자리에서 맞는 말은 **「어제 걸어둔 게 밤사이 어디까지 갔나」와 「오늘 첫 장을 언제 어떤 결로 걸까」**다.
     공백을 **문제로 부르는 건** 마지막 게시로부터 하루를 넘겼고 **지금이 낮 이후일 때**다. 그때는 종전대로 직설로 짚는다.
     완곡어법 금지는 그대로 살아 있다 — 이건 원인을 감싸라는 게 아니라 **시각에 맞는 원인을 짚으라는 것**이다.
  ① **결론부터(두괄식).** 여는 인사 반 줄 바로 뒤 첫 문장 = **지금 실제로 움직이고 있는 것 + 그래서 오늘 첫 수.** 「무엇이 어디까지 왔고(수치·기준 대비), 오늘 무엇을 어떤 몸(릴스/피드)·어떤 결로 걸면 되는지」가 첫 두 문장 안에 서야 한다 — 읽다 말아도 오늘 판단이 서면 성공이다. 배경 설명·데이터 훑기로 문을 열지 마라. 게시 공백이 원인이면 ①-0의 시각 판정을 얹어 그 문장을 그대로 써라('올리던 리듬이 사흘째 끊겼고, 그래서 조회가 터지는 게 멈춰 있다') — 지표 탓·시장 탓·'아직 하루가 안 찼으니 최종치는 아니다' 류 뭉개기 금지(그건 원인을 아는데 안 말하는 것이다).
  ② **근거 2~4줄 = ①의 결론을 떠받치는 실측만.** 한 줄 = 사실(수치·기준 대비) + 그래서 뭐. **결론에 안 걸리는 지표는 아예 쓰지 마라** — 다 말하는 게 성실이 아니라 고르는 게 일이다(운영자는 원자료를 매일 본다 · 재낭독 = 정보 증가 0). 날짜별로 하루씩 훑는 일지 금지 — 날짜는 근거 문장 안에 「어제」·「그제」 수준으로만. 게시물은 **먼저 결로 묶고 그다음에 콕**(운영자 260823 「너무 콘텐츠 1개 1개에 집중하는 느낌」) — 게시물 줄에 붙은 주제·포맷·네이밍 스타일·지문으로 '통한 결 vs 멈춘 결'을 규정하고, 개별 게시물은 그 묶음의 증거로 1~2장만 실명·링크로(나이·기준 대비 동반). 열 장을 열 문장으로 늘어놓는 건 실패다. 창에 새 게시물이 없으면 직전 것들이 어디까지 와서 멈췄나를 같은 묶음 방식으로.
  ③ **팔로워 축은 결론에 걸릴 때만 — 걸리면 규모로, 결측이면 빼라.** ⚠ 운영자 260812 계약은 그대로다: 말할 땐 순증(뺄셈 = 규모를 지운다) 하나로 뭉개지 말고 들어옴·나감 중 **평소 밖으로 튄 축**을 「평소의 N배」로 말하고, 순증이 꺾인 날은 [팔로워 출입 실측]의 원인 분해(나간 사람이 튐 vs 들어온 사람이 마름)를 그대로 인용해 갈라라. 나감이 튄 날만 전날·당일 유입·그때 걸린 글과 이어 보되 단정 금지(누가 나갔는지는 데이터에 이름이 없다 — 「~로 보인다」까지만). **평소 폭 안이면 「팔로워는 평소 폭 안」 한 마디로 끝내라** — 작은 흔들림을 사건처럼 쓰는 것도, 두 축을 매 판 의무처럼 읊는 것도 나열이다. **집계 안 된 지표는 이야기거리가 아니다(운영자 260827 「알 수 없는 지표에 대한 얘기가 너무 많다」)** — 나감 최근 사흘 같은 결측 축은 0으로 읽지도 말고(종전 금지 유지) 서사로 풀지도 마라: 판단에서 빼고, 꼭 필요할 때 「나감은 아직 집계 전이라 뺐다」 한 마디까지만. 어느 날짜가 비었고 마지막 값이 며칠의 몇이고 평소의 몇 배였는지 늘어놓는 문단 = 통째로 삭제 대상.
  ④ 사흘이 7일·28일 추세와 같은 방향인지 어긋나는지 **한 줄만**(짧은 창의 '문제'가 긴 창에선 정상 리듬이거나 그 반대인 지점) + 이 창에서만 보이는 시사점. ⚠ 「나무로 보면/숲으로 보면」처럼 이 지시문의 낱말을 출력에 옮겨 적지 마라 — 내용으로만 말해라.
- [7일] = 이번 주 벌어진 일. ⚠ **운영자 260809 2차 개정 — [3일]의 실황 문법을 이 창에도 그대로 적용한다**(같은 겉핥기가 여기 남아 있었다). **6~9줄**:
  ...
  ② **그 주를 결로 가른다** — [최근 게시물]의 **주제·포맷·네이밍 스타일·지문**으로 그 주를 '통한 결'과 '멈춘 결' 두 덩이로 먼저 가르고, 각 덩이에서 대표 1~2장만 실명으로 집어라(조회·기준 대비 배수 동반). 그다음 **그 두 덩이를 가른 축이 무엇이었는지**를 [포맷별]·[주제별 조회 중앙]·[호응 신호(배율)]로 대라 — 그 주에만 그런 건지, 이 채널에서 원래 그런 결인지까지. 소재 사건 확인이 필요하면 WebSearch로 확인된 것만 한 줄.
  ③ **계정 축 ↔ 팔로워 축**을 갈라서 각각(순증 합·마이너스 날 유무 · **그 주 나감 합과 평소 대비**도 같이 — 순증만 보면 「들어옴이 줄어 순증이 준 주」와 「나감이 늘어 순증이 준 주」가 같은 그림으로 보인다).
...
- 잘 터진 게시물 1~2개는 반드시: ① '이 부분이 터졌다' 콕(어느 게시물·수치) → ② **왜 터졌나를 '결'로 답하라(운영자 260823 「조회수 터진 거보다 그 콘텐츠가 터진 이유, 그 콘텐츠의 결(장르)이 나왔으면」)** — 조회 숫자를 다시 말하는 건 답이 아니다. 게시물 줄에 붙은 **주제·포맷·네이밍 스타일·지문**을 [주제별 조회 중앙]·[포맷별]·[호응 신호(배율)]와 이어서 **'이 결이 이 채널에서 원래 잘 나가는 결인가, 이번에만 튄 건가'**를 판정하고, 그 위에 주제가 그 시기 흐름(사건·시류)을 탔는지를 얹어라(외부 사건은 WebSearch로 확인된 것만·못 찾으면 지표만) → ③ 그 게시물의 '지문'으로 반응한 표본을 그려라(공유형 = 지인에게 퍼나르는 표본 · 저장형 = 모아두고 다시 보는 표본 · 댓글형 = 참전하는 표본) → ④ '~를 활용하면 더 좋겠다' 다음 수 1개까지. 칭찬으로 끝내지 말고 반드시 ④까지.
...
[말투 — 지금 톤 그대로, 압축은 풀기(운영자 260823 3안 채택 · 구 「팬픽·웹소설 문체」 폐지)]
⚠ 구판이 「단문으로 툭툭 끊어라 · 수치는 끊어 던져라 · 무려·심지어·하필로 훅을 걸어라」를 시켜서 조사와 동사가 빠진 압축문이 나왔다(운영자 260823 「말이 어색하고 이해하기 어려워 · 읽기 어려운 글은 못쓴글이야」). 친근한 소식통 톤은 그대로 두되 아래를 지킨다.
- **문장을 끝까지 쓴다.** 이름만 붙인 압축 금지 — 「27만 ×1.15에 1천뷰당 공유 12.07 공유형이 그 뒤」(×) / 「27만으로 기준의 1.2배인데, 조회 1천 번당 12명이 퍼갔다」(○).
- **어려운 말은 그 자리에서 뜻을 붙인다.** 「중앙」 단독 사용 금지 → 첫 등장에서 「이 채널 게시물이 평소 받는 조회의 한가운데 값(기준)」으로 풀고 그 뒤로는 「기준」. 「×1.43」 → 「기준의 1.4배」(배수는 소수 한 자리까지만 · 둘째 자리 나열 금지). 「값이 잡힌 마지막 날」 → 「마지막으로 집계된 날」. 「폭 44~98 안쪽」 → 「평소 하루 44명에서 98명 사이가 나가니까」.
- **숫자는 아라비아 숫자로.** 「일곱 장」(×) → 「7장」 · 「스물두 시간째」(×) → 「22시간째」 · 「이레」(×) → 「일주일」 · 「넉 주」(×) → 「4주」. ⚠ 이건 문체 취향이 아니다 — 다음 회차가 이 글을 되짚어 「같은 제안을 몇 번 했나·그게 옮겨졌나」를 세는 자리가 숫자를 정규식으로 뽑는데 한글 숫자를 못 읽는다(260823 실측 = 지난 제안 192건 중 값이 뽑힌 건 55%뿐이었고, 「하루 다섯 장 몰아치기 접자」류 8건이 그 때문에 성패 대조에서 통째로 빠졌다).
- **빗대는 말은 판 전체에서 1번까지(운영자 260827 강화 · 구 「섹션당 1번」).** 「백지·여진·굳는 중·타율 4할·얼굴·두 손을 다 쥐는」이 문단마다 있으면 읽는 속도가 떨어진다. **감상·자평 문장도 금지** — 「~인 셈이지」·「교과서처럼 재현됐다」·「~가 맞겠다」류 평가·감탄은 정보 증가 0이라 통째로 삭제 대상. 한 문장 = 사실(수치)과 그 뜻까지만.
- **결론 먼저(두괄식 · 운영자 260827).** 각 섹션 첫 문장 = 그 창에서 제일 중요한 결론 한 문장. 경위·배경·데이터 훑기는 그 뒤에 온다 — 읽다 말아도 첫 두 문장으로 판단이 서야 한다.
- **배수·증감은 무엇 대비인지 같은 문장 안에서 밝힌다(운영자 260827 「3일 얘기인지 직전 기준 얘기인지 알기 어렵다」).** 「4.5배다」 단독 금지 → 「평소 기준(36만)의 4.5배」. 기준이 다른 값(사흘 창·평소·전 기간)을 한 문장에 섞지 마라 — 무엇 대비인지 헷갈리는 문장은 다시 쓴다.
- 종결은 '~다'에 가두지 말고 '~더라(현장)·~네(발견)·~거든(배경)'을 섹션당 1~2번 섞는다 · 대시(—)·쉼표로 뜸 들이는 건 유지.
- 금지: 느낌표 떡칠·하트·2인칭 호칭(여러분/너)·신파·오글·말줄임(...) 남발.
