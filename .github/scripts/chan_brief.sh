    ...
    try:
        ...
        if len(_nn) >= 5:
            ...
            if len(_oo) >= 5:
                ...
                _ul = (f" || 나감 실측 {len(_uv)}일: 중앙 {_umd}명/일 · 폭 {min(_uv)}~{max(_uv)} · 표준편차 {_usd}"
                       f"{' = 거의 일정' if _usd < _umd * 0.3 else ''}"
                       f" / 들어옴 중앙 {_fmd if _fmd else '—'}명/일"
                       + (f" · ⚠최근 {_gap}일은 나감 미수집(집계 3일 지연 = 결측이지 0이 아니다)" if _gap else '')
                       + (f" || 꺾인 날 원인 분해: {' · '.join(_why[-5:])}" if _why else ''))
            L.append(f"[팔로워 출입 실측(최근 30일 · 순증 = 들어옴 빼기 나감)] "
                     f"순증 중앙 {('+' if _nmd > 0 else '')}{_nmd}명/일 · 평소 흔들림 ±{_mad}"
                     f"(= 이 폭 안이면 평범한 날 · 표준편차 대신 중앙절대편차 = 폭발일 1건에 자가 늘어나는 것 차단)"
                     + f" || {_lo3}" + _ul)
    except Exception:
        pass
...
- [3일] = 지난 사흘의 **실황 중계**. 여는 인사 한 줄로 시작. ⚠ **운영자 260809 개정 — '수박 겉핥기 · 게시물 하나하나의 디테일을 잡아달라 · 너무 돌려 말하니 유의미한 인사이트가 안 나온다'는 실측 지적.** 아래 4개를 전부 넣는다(**6~9줄**):
  ...
  ② **계정 축과 팔로워 축을 갈라서** 각각 한 줄 — 한쪽이 꺼져도 다른 쪽은 유지될 수 있고, 그 어긋남 자체가 이 창의 정보다(순증감 수치를 그대로 인용).
     ...
     ⓐ **크기를 말해라.** [팔로워 출입 실측]의 「평소 흔들림 ±N」과 대조해 그날이 평소 폭 안인지 **몇 배 밖인지** 붙여라 — 「-9」와 「-96」을 같은 말투로 쓰는 게 이 개정이 겨눈 사고다(작은 흔들림을 사건처럼 쓰는 것도 같은 죄다).
     ⓑ **원인을 갈라라.** 순증이 꺾였으면 **나간 사람이 튄 건지 들어온 사람이 마른 건지** 실측으로 말해라(둘은 처방이 정반대다 — 나감이 튀었으면 최근 올린 결을 보고, 들어옴이 말랐으면 도달이 멈춘 걸 본다). [팔로워 출입 실측]의 「꺾인 날 원인 분해」가 이미 갈라 놓았으니 그대로 인용하면 된다.
