...
TIMEOUT_S = int(os.environ.get("GATE_TIMEOUT", "300"))        # 콜당 상한(s) — **opus + effort 운영(260810 max → 260923 high)에서는 반드시 상향**(sonnet 기준 300은 추론 붙은 콜에 짧다 · 초과 = rc≠0 = 미도장 = 다음 런 재시도라 조용한 무한 재시도로 굳는다). 워크플로 env GATE_TIMEOUT 로 부여.
...
REGRADE_HEAD_RE = re.compile(r"^\s*[\[\(](표|[A-Z]{0,2}포토|사진|그래픽|알림|공고|부고|부음|동정|인사|프로필|단신|게시판)")   # 정형 머리표 = 뷰어 JUNK_HEAD가 양 칼럼서 숨기는 부류 → 재채점 이득 0(첫 드라이런 실측: [표] 경선투표 cr17·[포토] 삭발식 cr9가 큐 진입 = 콜 낭비)
...
- 한국 언론 관행 표기(인명·지명·기관 — 예: Trump→트럼프 · Gaza→가자지구), 숫자·수치·인용 발언 정확히 보존, 의미 왜곡·과장·내용 추가/삭제·번역투 금지(피동 '~에 의해'·'~에 대해/~를 통해' 남발 = 조사로 직결 · 정본 shared/ko_tone_rules.md 문장축).
...
[3 = 대형·엄중 — 누가 봐도 중요]
⚠️ **[3] 판정법 — 제목만 보고 한국 사람 대부분이 "와 씨, 진짜?" 하고 놀라 주변에 말을 꺼낼 소식이면 [3], 아니면 [2] 이하**(운영자 260925 · 목적 = 중요한 뉴스 가운데 진짜 큰 이슈만 거르기 · ⚠️ 이 판정법은 [3]을 줄지 말지에만 쓴다 — [0]·[1]·[2] 사이 경중은 아래 조항대로이고 이걸로 [1]을 [2]로 올리지 마라). 그 반응은 아래 두 축이 겹칠 때 나온다 — **둘 다 높을 때만 [3]**, 하나만 높으면 [2] 이하:
 Ⓐ 도달 — 얼마나 많은 사람과 관련 있나: 한국인 대부분이 알거나 세계적으로 이름난 대상(인물·조직·제도·물건)이 당사자인가 / 영향이 특정 지역·집단이 아니라 국민 대다수의 몸·안전·권리에 곧바로 닿는가 / 불특정 다수가 당할 수 있었던 일인가 / 온 국민이 같은 감정(환호·분노·공포·슬픔)으로 반응할 국가 단위 일인가.
 Ⓑ 충격 — 얼마나 평소와 다른가: 처음이거나 역사에 남을 일인가 / 인명·피해·위기의 규모가 평소 보도되는 사건의 범위를 크게 넘는가(그 나라에서 전례 없는 규모 포함) / 사람들이 당연하게 믿던 것(안전·공정·권위)이 깨졌는가 / 극적인 반전·몰락이거나, 최상위 인물이 관례·의전을 깨고 평소 하지 않는 행동을 했는가.
 ⓒ 적용 — 이름난 대상의 평범한 일정·발언·근황·홍보(도달만 높음)와, 멀리서 벌어진 낯선 사건(충격만 높음)은 [3]이 아니다. 흔히 반복되는 유형(개별 화재·교통·산업 사고, 개인 간 범죄, 정치 공방, 행정 절차, 실적)은 아래 ⚖ 인명 문턱이 충격의 하한선이다. 아래 조항이 유형을 [0]·[1]로 정했어도 두 축이 모두 최상인 국가 단위 사건이면 [3]이고, 조항이 '폭·규모와 무관하게'로 못박은 유형(증시·환율 시황 등)은 그대로 둔다. 조항에 딱 맞는 사례가 없으면 이 판정법으로 판단하고, [3]인지 애매하면 [3]을 주지 않는다.
- 사고·화재·재난·폭발·붕괴·추락·침몰 등 돌발 사고 — **아래 ⚖ 인명 문턱 ②(국내 확정 사망 3명 이상 · 해외 사망 10명·부상 50명·실종 150명 이상)가 제목에 드러난 것만 [3]**(운영자 260925 "그 외에는 3명"·"해외는 속보 해외 문턱" = 260826 "3등급은 사망 포함해야"의 상향). ⚠️ **사망 1~2명·심정지·'N명 사상'은 [3] 금지 → [2]**(단 아래 [1] 조항의 일상 단일 사고사·변사 1명은 [1] · 화재·폭발·붕괴 사망은 일상 사고사가 아니다 = [2]). ⚠️ 단 **도심·다중이용시설의 폭탄 폭발·테러는 사람 수와 무관하게 [3]**(사건 자체가 전국적 이슈 · 운영자 260925). ⚠️ **규모만으로는 [3] 금지 → [2]** — 대형 공장·산단·시장·산림 화재, 건물 전소, 대규모 대피, 붕괴·폭발이어도 문턱을 넘는 사망이 제목에 없으면 [2]다(속보 판정의 🔥 화재 인명 축과 같은 국내외 문턱 = 물적 피해·대응 상황만으로는 안 올린다). ⚠️ **사상 여부가 제목에 안 드러난 초기 속보도 [3] 금지 → [2]**('진화 중'·'조사 중'·'대응단계 발령' 등 — 문턱을 넘는 사망이 확인되면 그 후속 보도가 새로 [3]을 받는다 = 놓치지 않는다). 예: '광양 도축장서 불…소방 진화 중' = [2] / '쿠팡 물류센터 화재…직원 등 3000여명 대피' = [2] / '다세대주택 화재…70대 1명 숨져' = [2](사망 1명) / '경기 양주 공장서 불…1명 심정지 구조' = [2](심정지 = 사망 아님) / '국내 아파트 화재로 일가족 4명 숨져' = [3] / '인도 버스 추락 8명 사망' = [2](해외 10명 미달) / '중앙아프리카공화국서 불법 금광 붕괴로 최소 100명 사망' = [3]
- 전쟁·국가간 무력충돌의 발발·새 국면·국가급 전개(개전·첫 본토 공습·포격 도발·포로 송환 등 — 사망 수와 무관하게 [3]), 요인·유명인 피습·암살(피해자 1명·부상만이어도 — 위 [3] 판정법대로 **피해자가 누구냐**로 판단: 전국적 이슈가 될 인물이면 [3] · 정치인은 대략 의전상 장관급 이상[여야 정당 대표 포함], 그 밖의 인물은 국내외 인지도[세계 최상위 기업·기술을 이끄는 인물 등]로 · 이슈가 안 될 인물이면 [2] · 운영자 260925), 충격적 사건 — ⚠️ 진행 중인 전쟁의 개별 피해·테러·강력범죄·사고처럼 사람 수로 무게가 정해지는 사건은 아래 ⚖ 인명 문턱이 우선이다('충격'·'참사'·'발칵' 같은 수식어로 문턱을 넘기지 않는다)
- **감염병·전염병의 대규모 확산** — 단 **실제 사망이 아래 ⚖ 인명 문턱 ②(국내 확정 사망 3명·해외 사망 10명 이상)를 넘은 경우만**(운영자 260810 "감염병은 피해자가 발생했을때만임" · 260925 "그 외에는 3명"·"해외는 속보 해외 문턱"). 예: '민주콩고 변종 에볼라 역대 두 번째 규모로 확산…3개월 만에 사망자 1556명' = [3]. ⚠️ 발생 확인·유입 차단·주의보·예방 접종·감염 우려 단계는 [3] 아님 = 원래 경중대로 [2](돌발 사고와 달리 서서히 번지는 축이라 「초기 속보 보호」 대상도 아니다)
...
⚖ **[3] 인명 문턱 = 흔한 사건의 충격 하한선**(운영자 260925 "살인 성폭행은 2명 기준, 그 외에는 3명" · "해외는 속보 해외 문턱" · "살인 부상자는 애초에 기준으로 하지 않음" · "성폭행 자체로 G3는 될 수 없음, 유명인이 낀 게 아니라면") — 사람이 죽거나 다친 것으로 무게가 정해지는 사건은 **제목에 명시된 수**가 이 문턱을 넘어야 [3]이다. 미달이면 [3] 금지 → [2](아래 [1] 조항에 해당하면 [1]). ⚠️ 이 문턱은 흔히 반복되는 사건이 [3]으로 부풀지 않게 거르는 하한선이다 — 전례 없는 사건·대형 테러·수백 명 규모의 생명 위기·초대형 자연재해처럼 위 [3] 판정법의 두 축이 모두 최상인 사건은 사망 수가 미달·미상이어도 [3]. ⚠️ 이 문턱은 사람 수(사망·피해자)로 무게가 정해지는 사건에만 쓴다 — 연예 예외·지위 변동, 정치·외교·전쟁의 발발·새 국면·국가급 전개·요인 피습·도심 폭탄 테러·제도 축의 [3]은 이 문턱과 무관하게 각 조항대로 판정하고, 제목의 숫자가 사상자 수가 아니면(포로·인원 이동·인사 등) 세지 않는다.
 ① **살인 = 숨진 피해자 2명 이상**(국내·해외 동일). 살인 = 사람이 고의로 사람을 죽인 사건(흉기·총격·테러·타인을 노린 방화 살인 포함). ⚠️ **숨진 사람만 센다 — 부상자·중상자·살인미수 피해자는 인원과 무관하게 세지 않는다**('흉기 난동…5명 사상' = 사망 수 미상 → [2]). ⚠️ 국가·군의 공습·포격·미사일 피해는 ①이 아니라 ②다. 살인임이 제목에 드러나야 ①이다 — 원인·대상 미상의 '숨진 채 발견'·'방화 추정' 화재 사망은 ②로 센다. ①은 사건이 처음 드러나는 보도(검거·구속으로 처음 알려진 경우 포함)에 적용하고, 이미 알려진 사건의 기소·구형·선고·항소심은 아래 개별 형사사건 조항대로.
 ⚠️ **성폭행 = 피해자 2명 이상이면 [3]**(①과 같은 문턱 · 운영자 260925 "살인 성폭행은 2명 기준"). 피해자 1명 단건은 그 자체로 [3]이 될 수 없다 — 유명인이 낀 경우만 아래 연예 예외·유명인 조항대로. 불법촬영·딥페이크·성추행은 이 문턱 대상이 아니다(원래 경중대로).
 ② **그 외(사고·화재·재난·폭발·붕괴·추락·침몰·감염병·진행 중 전쟁/국제충돌 피해 등)** — **국내 = 확정 사망 3명 이상** / **해외 = 사망 10명 이상 OR 부상 50명 이상 OR 실종 150명 이상**(속보 판정 ⚖ 해외 문턱과 동일 · 세 수는 각각 독립이고 합산하지 않는다 · 해외 공습·포격 등 군사행위 피해는 사망 10명 이상만). 살인이 아니면 국내도 사망 2명 이하는 [3]이 될 수 없다.
 공통 셈법 — ⓐ 사망은 확정 사망만 센다: 심정지·의식불명·부상·병원 이송·실종·매몰·고립은 사망이 아니다(해외 부상·실종 문턱은 부상·실종으로 명시된 인원만). '사상자 N명'·'인명피해 N명'은 사망·부상 구분 미상이라 어느 칸에도 세지 않는다. ⓑ 가해자·용의자(숨진 범인 포함)는 세지 않는다 — '아내 살해 후 극단선택…부부 숨진 채'는 피해자 1명. ⓒ 한 사건의 피해만 센다 — 여러 날·여러 건을 모은 기간 집계('올여름 온열질환 사망 7명'·'두 달 새 N명 피살')는 문턱 계산에 넣지 않는다(하루 안의 교전·공격·재난 피해는 한 사건으로 센다 — '하루 새 44명 숨져' = 44명). ⓓ '모녀·부부·일가족 N명 숨져'처럼 관계로 적힌 수는 그 인원으로 센다(모녀 = 2명) — 인원수 없는 '일가족'은 세지 않는다. 예: '귀갓길 모녀 살해한 50대 검거' = [3](①) / '제자 3명 성폭행한 학원강사 구속' = [3](성폭행 피해자 2명 이상) / '추석 연휴 첫날 옥탑방서 화재...60대 男 사망' = [2](②미달) / '[속보] 경기 양주 덕계동 빌라서 화재…7명 사상' = [2](사망 수 미상)

[2 = 주목 — 일반적으로 중요한 뉴스]
- 주요 정치·국제·외교·정책·사회 현안(일상 행정 넘어선 것)
- 의미 있는 경제·산업·사회 변화
- 조직적 범죄·공공 위험의 적발·일망타진(다수에 닿는 위험·대규모 조직 검거 — 예: '악어·코브라 200마리 밀수 조직 덜미' = [2] · 개별 단건 사건은 아래 [1] 규정대로)
- 대규모 군중 급박위험 — 다중이용시설·대규모 인파(해수욕장·경기장·축제·역·공항 등)에 대한 즉각적 물리위협(위험동물·유독물질 누출·붕괴/폭발 임박 등)에 실시간 대응(대피·현장통제·시설폐쇄, 또는 다발 목격·잇단 신고·경보 등 급박 대응 정황)이 진행 중인 것. 아직 대량 인명피해는 없어도 다수에게 즉각 위험이면 [2]. ⚠️ 확정 사망·피해자 수가 위 ⚖ 인명 문턱을 넘었으면 [3]. 개별·소수 대상, 정기 안전점검·예방 캠페인·계도·단순 주의 안내·일상 기상주의보(폭염·미세먼지 등)면 [1] — 승격 요건 = '대규모 인파 노출 × 즉각적 물리위협 × 실시간 급박 대응' 모두 충족.
...
- **증시·환율 시황의 등락 = 폭 무관 [1]** — 폭락·급락·서킷브레이커·사이드카·상하한가 포함(운영자 260807: 레버리지 상품 도입 후 급등락·서킷브레이커가 빈번해져 영향도와 무관하게 보도 임팩트가 희석됨 — "주식은 [2] 택도 없다"). 단순 마감 시세·수치 나열은 종전대로 [0]. 금융시스템 자체의 사건(뱅크런·거래 전면 중단 장기화·국가 디폴트)만 별개 — 원래 경중대로. ⚠️ **등락에 붙는 시장 전반의 통상 조치는 '처분' 조항이 아니다 = 시황 그대로 [1]**(상·하한가 폭 제한·확대·한시 금지, 변동성 완화장치, 프리마켓·시간외 거래 일시정지 등 — 위 실적 조항 ①의 '당국·거래소의 처분·결정'은 **특정 종목·기관을 겨눈 처분**[상장폐지 결정·매매정지 처분·공적 제재]만을 뜻한다). 예: '‘하닉 1주에 하한가’…프리마켓 상·하한가 한시 금지' = [1](260826 실측 = 이 두 줄이 같은 제목에서 충돌해 판정이 회차마다 흔들리던 자리 · 운영자 260807 "주식은 [2] 택도 없다" 방침 계승 = 새 값 0)
...
⚠️ 개별·단일 피의자 형사사건의 선고·판결·구형·항소심·구속(전국적 대형·화제 아닌 일상 강력범죄)은 **[3] 아님** — 개별 사법 절차 결과는 일반 기사로 **기본 [1]**에 둔다(단 위 ⚖ 인명 문턱 ①[살인 숨진 피해자·성폭행 피해자 2명 이상]을 넘는 사건이 처음 드러나는 검거·구속 보도는 [3] · 전국적 대형·화제·고위공직자급, 또는 **유명인·유명 기업인·셀럽(대중이 알 만한 이름)이 당사자**일 때 [2] — 유명인 사건은 주장·폭로 단계든 피소·구속 절차 단계든 동일 급(운영자 260805 "철구·구속도 400억과 동일 급") · breaking 긴급 판정과 동일 기준·260618·260628 엄선).

⚠️ 화재·사고라도 ① 제목에 '인명피해 없음·재산피해만·연기만'이 명시됐거나 ② 사건성 없는 일상적 단일 사고사·변사(개별 추락·질식·끼임·교통·돌연사 등 1명)면 **[3]·[2] 아님 → [1]**(사건이 벌어진 사실과 별개로 *피해 규모*로 경중을 매김). 단 다음은 깎지 말고 유지 — ⓐ 사상 여부가 제목에 안 드러난 화재·사고·재난(초기 속보 보호 — ⚠️ 이 보호는 **[2]까지이고 [3]은 금지**, 위 ⚖ 인명 문턱대로 · 운영자 260826·260925) ⓑ 사상 2명 이상·심정지(=다수) ⓒ 타살·피살·총격·테러·전쟁/국제충돌 피해·산업재해/근무 중 사망·아동/취약자 피해·고위공직자/유명인·전국적 화제(발칵·파문·공분 등)는 단일이어도 '일상' 아님(260620 엄선) ⓓ 대규모 인파 대상 즉각적 물리위협에 실시간 대응(대피·현장통제·폐쇄 또는 다발 신고·목격·경보)이 진행 중인 것(대량 피해 0이어도 위 [2] '대규모 군중 급박위험' — 260704). ⚠️ ⓐ~ⓓ는 [1]로 깎지 않는 보호일 뿐 [3] 자격이 아니다 — [3]은 위 ⚖ 인명 문턱을 넘어야 한다(260925).
...
def main():
    ...
    # 룰북 개정 재채점분 = 채점 시점 cross 로 사다리 단 소진 도장(운영자 260925 「한 수 적용」) — 구판은 regraded* 도장이
    # RUBRIC_VER 에 묶여, 룰북 한 줄 수정 → 일반 재채점(이미 cross 6+ · 새 rubric) 직후 사다리가 같은 제목·같은 rubric 으로
    # 한 번 더 콜했다(260925 후보 스냅샷 계산 = 재채점 391건 중 176건 중복 · 콜 약 +40%). 사다리의 존재 이유는 「낮은 cross 에서
    # 매긴 등급을 매체가 붙은 뒤 다시 보기」라, 그 단의 cross 에서 새 rubric 으로 막 매긴 건은 다시 볼 정보가 없다.
    # 첫 채점(grade 없음)은 무접촉 = 기존 사다리 동작 그대로 · 이후 cross 가 다음 단으로 자라면 사다리는 종전대로 1회 돈다.
    rejudge_ids = {id(c) for c in surf if c.get("grade") in (0, 1, 2, 3) and c.get("grade_rubric") != RUBRIC_VER}
    ...
    for i, c in enumerate(pending):
        ...
        if surfaced(c):                   # 노출권 = grade+cat 둘 다 반영(기존 동작)
            ...
            if id(c) in regr_ids or id(c) in rejudge_ids:
                st = regrade_stage(c)         # 재채점 소진 도장 — 단별(rubric 세대당 1회 · 응답 온 줄만 = 실패분은 다음 런 재시도)
                if st:                        # 룰북 개정 재채점분은 cross 가 사다리 단 밑이면 도장 없음(자라면 종전대로 사다리)
