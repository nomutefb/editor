...
[3 = 대형·엄중 — 누가 봐도 중요]
- 사고·화재·재난·폭발·붕괴·추락·침몰 등 돌발 사고 — **아래 ⚖ 인명 문턱 ②(확정 사망 3명 이상)가 제목에 드러난 것만 [3]**(운영자 260925 "그 외에는 3명" = 260826 "3등급은 사망 포함해야"의 상향). ⚠️ **사망 1~2명·심정지·'N명 사상'은 [3] 금지 → [2]**(단 아래 [1] 조항의 일상 단일 사고사·변사 1명은 [1] · 화재·폭발·붕괴 사망은 일상 사고사가 아니다 = [2]). ⚠️ **규모만으로는 [3] 금지 → [2]** — 대형 공장·산단·시장·산림 화재, 건물 전소, 대규모 대피, 붕괴·폭발이어도 문턱을 넘는 사망이 제목에 없으면 [2]다(속보 판정의 🔥 화재 인명 축과 같은 국내 문턱 = 물적 피해·대응 상황만으로는 안 올린다). ⚠️ **사상 여부가 제목에 안 드러난 초기 속보도 [3] 금지 → [2]**('진화 중'·'조사 중'·'대응단계 발령' 등 — 문턱을 넘는 사망이 확인되면 그 후속 보도가 새로 [3]을 받는다 = 놓치지 않는다). 예: '광양 도축장서 불…소방 진화 중' = [2] / '쿠팡 물류센터 화재…직원 등 3000여명 대피' = [2] / '다세대주택 화재…70대 1명 숨져' = [2](사망 1명) / '경기 양주 공장서 불…1명 심정지 구조' = [2](심정지 = 사망 아님) / '국내 아파트 화재로 일가족 4명 숨져' = [3] / '중앙아프리카공화국서 불법 금광 붕괴로 최소 100명 사망' = [3]
...
- **감염병·전염병의 대규모 확산** — 단 **실제 사망이 아래 ⚖ 인명 문턱 ②(확정 사망 3명 이상)를 넘은 경우만**(운영자 260810 "감염병은 피해자가 발생했을때만임" · 260925 "그 외에는 3명"). 예: '민주콩고 변종 에볼라 역대 두 번째 규모로 확산…3개월 만에 사망자 1556명' = [3]. ⚠️ 발생 확인·유입 차단·주의보·예방 접종·감염 우려 단계는 [3] 아님 = 원래 경중대로 [2](돌발 사고와 달리 서서히 번지는 축이라 「초기 속보 보호」 대상도 아니다)
...
⚖ **[3] 인명 문턱**(운영자 260925 "살인 성폭행은 2명 기준, 그 외에는 3명") — 사람이 죽거나 다친 것으로 무게가 정해지는 사건은 **제목에 명시된 수**가 이 문턱을 넘어야 [3]이다. 미달이면 [3] 금지 → [2](아래 [1] 조항에 해당하면 [1]). ⚠️ 이 문턱은 사람 수(사망·피해자)로 무게가 정해지는 사건에만 쓴다 — 연예 예외·지위 변동, 정치·외교·전쟁의 발발·새 국면·국가급 전개·요인 피습·제도 축의 [3]은 이 문턱과 무관하게 각 조항대로 판정하고, 제목의 숫자가 사상자 수가 아니면(포로·인원 이동·인사 등) 세지 않는다.
 ① **살인·성폭행 = 직접 피해자 2명 이상**(속보 판정 🔪 대인 강력범죄 게이트와 같은 셈법). 살인 = 사람이 고의로 사람을 죽이거나 죽이려 한 사건(살인미수·흉기 난동·총격·테러·타인을 노린 방화 포함)이고, 직접 피해자 = 숨진 사람·중상자(①에서 '사상자 N명'·'N명 사상'은 N명을 직접 피해자로 센다). 성폭행 = 강간 등 성폭행 피해자 수(사망과 무관). ⚠️ 불법촬영·딥페이크·성추행은 ①이 아니다 = 원래 경중대로. ⚠️ 국가·군의 공습·포격·미사일 피해는 ①이 아니라 ②다. 살인·성폭행임이 제목에 드러나야 ①이다 — 원인·대상 미상의 '숨진 채 발견'·'방화 추정' 화재 사망은 ②로 센다. ①은 사건이 처음 드러나는 보도(검거·구속으로 처음 알려진 경우 포함)에 적용하고, 이미 알려진 사건의 기소·구형·선고·항소심은 아래 개별 형사사건 조항대로.
 ② **그 외(사고·화재·재난·폭발·붕괴·추락·침몰·감염병·진행 중 전쟁/국제충돌 피해 등) = 확정 사망 3명 이상.** 국내·해외 동일.
 공통 셈법 — ⓐ ②는 확정 사망만 센다: 심정지·의식불명·부상·병원 이송·실종·매몰·고립은 사망이 아니고, '사상자 N명'·'인명피해 N명'은 사망 수 미상이라 세지 않는다. ⓑ 가해자·용의자(숨진 범인 포함)는 세지 않는다 — '아내 살해 후 극단선택…부부 숨진 채'는 피해자 1명. ⓒ 한 사건의 피해만 센다 — 여러 날·여러 건을 모은 기간 집계('올여름 온열질환 사망 7명'·'두 달 새 N명 피살')는 문턱 계산에 넣지 않는다(하루 안의 교전·공격·재난 피해는 한 사건으로 센다 — '하루 새 44명 숨져' = 44명). ⓓ '모녀·부부·일가족 N명 숨져'처럼 관계로 적힌 수는 그 인원으로 센다(모녀 = 2명) — 인원수 없는 '일가족'은 세지 않는다. 예: '귀갓길 모녀 살해한 50대 검거' = [3](①) / '제자 3명 성폭행한 학원강사 구속' = [3](①) / '추석 연휴 첫날 옥탑방서 화재...60대 男 사망' = [2](②미달) / '[속보] 경기 양주 덕계동 빌라서 화재…7명 사상' = [2](사망 수 미상)
...
⚠️ 개별·단일 피의자 형사사건의 선고·판결·구형·항소심·구속(전국적 대형·화제 아닌 일상 강력범죄)은 **[3] 아님** — 개별 사법 절차 결과는 일반 기사로 **기본 [1]**에 둔다(단 위 ⚖ 인명 문턱 ①[살인·성폭행 피해자 2명 이상]을 넘는 사건이 처음 드러나는 검거·구속 보도는 [3] · 전국적 대형·화제·고위공직자급, 또는 **유명인·유명 기업인·셀럽(대중이 알 만한 이름)이 당사자**일 때 [2] — 유명인 사건은 주장·폭로 단계든 피소·구속 절차 단계든 동일 급(운영자 260805 "철구·구속도 400억과 동일 급") · breaking 긴급 판정과 동일 기준·260618·260628 엄선).
