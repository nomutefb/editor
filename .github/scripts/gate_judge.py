...
⚖ **[3] 인명 문턱**(운영자 260925 "살인은 2명 기준, 그 외에는 3명" · "해외는 속보 해외 문턱" · "살인 부상자는 애초에 기준으로 하지 않음" · "성폭행 자체로 G3는 될 수 없음, 유명인이 낀 게 아니라면") — 사람이 죽거나 다친 것으로 무게가 정해지는 사건은 **제목에 명시된 수**가 이 문턱을 넘어야 [3]이다. 미달이면 [3] 금지 → [2](아래 [1] 조항에 해당하면 [1]). ⚠️ 이 문턱은 사람 수(사망·피해자)로 무게가 정해지는 사건에만 쓴다 — 연예 예외·지위 변동, 정치·외교·전쟁의 발발·새 국면·국가급 전개·요인 피습·제도 축의 [3]은 이 문턱과 무관하게 각 조항대로 판정하고, 제목의 숫자가 사상자 수가 아니면(포로·인원 이동·인사 등) 세지 않는다.
 ...
 ⚠️ **성폭행·성범죄는 피해자 수와 무관하게 그 자체로 [3]이 될 수 없다** — 유명인이 가해자·피해자로 낀 경우만 아래 연예 예외·유명인 조항대로 판정하고, 무명 사건은 원래 경중대로([2] 이하).
...
⚠️ 개별·단일 피의자 형사사건의 선고·판결·구형·항소심·구속(전국적 대형·화제 아닌 일상 강력범죄)은 **[3] 아님** — 개별 사법 절차 결과는 일반 기사로 **기본 [1]**에 둔다(단 위 ⚖ 인명 문턱 ①[살인 숨진 피해자 2명 이상]을 넘는 사건이 처음 드러나는 검거·구속 보도는 [3] · 전국적 대형·화제·고위공직자급, 또는 **유명인·유명 기업인·셀럽(대중이 알 만한 이름)이 당사자**일 때 [2] — 유명인 사건은 주장·폭로 단계든 피소·구속 절차 단계든 동일 급(운영자 260805 "철구·구속도 400억과 동일 급") · breaking 긴급 판정과 동일 기준·260618·260628 엄선).
