...
[3 = 대형·엄중 — 누가 봐도 중요]
- 사고·화재·재난·폭발·붕괴·추락·침몰 등 돌발 사고 — **아래 ⚖ 인명 문턱 ②(확정 사망 3명 이상)가 제목에 드러난 것만 [3]**(운영자 260925 "그 외에는 3명" = 260826 "3등급은 사망 포함해야"의 상향). ⚠️ **사망 1~2명·심정지·'N명 사상'은 [3] 금지 → [2]**. ⚠️ **규모만으로는 [3] 금지 → [2]** — 대형 공장·산단·시장·산림 화재, 건물 전소, 대규모 대피, 붕괴·폭발이어도 문턱을 넘는 사망이 제목에 없으면 [2]다(속보 판정의 🔥 화재 인명 축과 같은 국내 문턱 = 물적 피해·대응 상황만으로는 안 올린다). ⚠️ **사상 여부가 제목에 안 드러난 초기 속보도 [3] 금지 → [2]**('진화 중'·'조사 중'·'대응단계 발령' 등 — 문턱을 넘는 사망이 확인되면 그 후속 보도가 새로 [3]을 받는다 = 놓치지 않는다). 예: '광양 도축장서 불…소방 진화 중' = [2] / '쿠팡 물류센터 화재…직원 등 3000여명 대피' = [2] / '양주 빌라서 방화 추정 화재 발생…집 안서 남성 숨진 채 발견' = [2](사망 1명) / '경기 양주 공장서 불…1명 심정지 구조' = [2](심정지 = 사망 아님) / '국내 아파트 화재로 일가족 4명 숨져' = [3] / '중앙아프리카공화국서 불법 금광 붕괴로 최소 100명 사망' = [3]
- 전쟁 발발·국가간 무력충돌의 새 국면, 충격적 사건 — ⚠️ 사람이 죽거나 다친 것으로 무게가 정해지는 사건(테러·강력범죄·전쟁/충돌 피해 포함)은 아래 ⚖ 인명 문턱이 우선이다('충격'·'참사'·'발칵' 같은 수식어로 문턱을 넘기지 않는다)
...
⚖ **[3] 인명 문턱**(운영자 260925 "살인 성폭행은 2명 기준, 그 외에는 3명") — 사람이 죽거나 다친 것으로 무게가 정해지는 사건은 **제목에 명시된 수**가 이 문턱을 넘어야 [3]이다. 미달이면 [3] 금지 → [2](아래 [1] 조항에 해당하면 [1]). ⚠️ 이 문턱은 사람 수(사망·피해자)로 무게가 정해지는 사건에만 쓴다 — 연예 예외·지위 변동, 정치·외교·전쟁 국면·제도 축의 [3]은 이 문턱과 무관하게 각 조항대로 판정하고, 제목의 숫자가 사상자 수가 아니면(포로·인원 이동·인사 등) 세지 않는다.
 ① **살인·성폭행 = 피해자 2명 이상.** 살인 = 사람이 고의로 사람을 죽인 사건(피살·살해·흉기·총격·방화 살인·테러 포함)이고 숨진 피해자만 센다 — 단 총기 난사·무차별 흉기 난동·테러처럼 불특정 다수를 노린 공격은 다친 사람도 피해자로 센다. 성폭행(성폭력·강간 포함)은 사망과 무관하게 피해자 수로 센다. 살인·성폭행임이 제목에 드러나야 ①이다 — 원인 미상의 '숨진 채 발견'·'방화 추정' 화재 사망은 ②로 센다. ①은 사건이 처음 드러나는 보도(검거·구속으로 처음 알려진 경우 포함)에 적용하고, 이미 알려진 사건의 기소·구형·선고·항소심은 아래 개별 형사사건 조항대로.
 ② **그 외(사고·화재·재난·폭발·붕괴·추락·침몰·감염병·전쟁/국제충돌 피해 등) = 확정 사망 3명 이상.** 국내·해외 동일.
 공통 — 확정 사망만 사망으로 센다: 심정지·의식불명·중상·부상·병원 이송·실종·매몰·고립은 사망이 아니다(①의 불특정 다수 공격만 다친 사람을 센다). '사상자 N명'·'인명피해 N명'은 사망 수 미상 = 사망으로 세지 않는다. '모녀·부부·일가족 N명 숨져'처럼 관계로 적힌 수도 그 인원으로 센다(모녀 = 2명). 예: '흉기로 이웃 2명 살해한 50대 검거' = [3](①) / '제자 3명 성폭행한 학원강사 구속' = [3](①) / '추석 연휴 첫날 옥탑방서 화재...60대 男 사망' = [2](②미달) / '[속보] 경기 양주 덕계동 빌라서 화재…7명 사상' = [2](사망 수 미상)

[2 = 주목 — 일반적으로 중요한 뉴스]
- 주요 정치·국제·외교·정책·사회 현안(일상 행정 넘어선 것)
- 의미 있는 경제·산업·사회 변화
- 조직적 범죄·공공 위험의 적발·일망타진(다수에 닿는 위험·대규모 조직 검거 — 예: '악어·코브라 200마리 밀수 조직 덜미' = [2] · 개별 단건 사건은 아래 [1] 규정대로)
- 대규모 군중 급박위험 — 다중이용시설·대규모 인파(해수욕장·경기장·축제·역·공항 등)에 대한 즉각적 물리위협(위험동물·유독물질 누출·붕괴/폭발 임박 등)에 실시간 대응(대피·현장통제·시설폐쇄, 또는 다발 목격·잇단 신고·경보 등 급박 대응 정황)이 진행 중인 것. 아직 대량 인명피해는 없어도 다수에게 즉각 위험이면 [2]. ⚠️ 실제 인명피해가 위 ⚖ 인명 문턱을 넘었으면 [3]. 개별·소수 대상, 정기 안전점검·예방 캠페인·계도·단순 주의 안내·일상 기상주의보(폭염·미세먼지 등)면 [1] — 승격 요건 = '대규모 인파 노출 × 즉각적 물리위협 × 실시간 급박 대응' 모두 충족.
