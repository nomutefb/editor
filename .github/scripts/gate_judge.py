...
[3 = 대형·엄중 — 누가 봐도 중요]
- 사고·화재·재난·폭발·붕괴·추락·침몰 등 돌발 사고 — **사람이 죽은 것이 제목에 드러난 것만 [3]**(사망·숨져·사망자·심정지·참사 등 · 운영자 260826 "3등급은 사망 포함해야"). ⚠️ **규모만으로는 [3] 금지 → [2]** — 대형 공장·산단·시장·산림 화재, 건물 전소, 대규모 대피, 붕괴·폭발이어도 사망이 제목에 없으면 [2]다(속보 판정의 🔥 화재 인명 축과 같은 기준 = 물적 피해·대응 상황만으로는 안 올린다). ⚠️ **사상 여부가 제목에 안 드러난 초기 속보도 [3] 금지 → [2]**('진화 중'·'조사 중'·'대응단계 발령' 등 — 사망이 확인되면 그 후속 보도가 새로 [3]을 받는다 = 놓치지 않는다). 예: '광양 도축장서 불…소방 진화 중' = [2] / '쿠팡 물류센터 화재…직원 등 3000여명 대피' = [2] / '경기 양주 공장서 불…1명 심정지 구조' = [3](심정지 = 위 ⓑ 조항이 다수급으로 못박은 축) / '중앙아프리카공화국서 불법 금광 붕괴로 최소 100명 사망' = [3]
...
⚠️ 화재·사고라도 ① 제목에 '인명피해 없음·재산피해만·연기만'이 명시됐거나 ② 사건성 없는 일상적 단일 사고사·변사(개별 추락·질식·끼임·교통·돌연사 등 1명)면 **[3]·[2] 아님 → [1]**(사건이 벌어진 사실과 별개로 *피해 규모*로 경중을 매김). 단 다음은 깎지 말고 유지 — ⓐ 사상 여부가 제목에 안 드러난 화재·사고·재난(초기 속보 보호 — ⚠️ 이 보호는 **[2]까지이고 [3]은 금지**, 위 [3] 조항의 사망 요건대로 · 운영자 260826) ⓑ 사상 2명 이상·심정지(=다수) ⓒ 타살·피살·총격·테러·전쟁/국제충돌 피해·산업재해/근무 중 사망·아동/취약자 피해·고위공직자/유명인·전국적 화제(발칵·파문·공분 등)는 단일이어도 '일상' 아님(260620 엄선) ⓓ 대규모 인파 대상 즉각적 물리위협에 실시간 대응(대피·현장통제·폐쇄 또는 다발 신고·목격·경보)이 진행 중인 것(대량 피해 0이어도 위 [2] '대규모 군중 급박위험' — 260704).
