...
";; esac
    ...
    case "$pres" in
      *h24*|*fp*|*mj*) PRESET_BLOCK="${PRESET_BLOCK} - 검색 상한 확인: 프리셋 검색(외신 영문 + 국내 교차)도 기본 보강 상한과 같은 총 3회 예산 안에서 수행한다(260825 하향 — 구 '프리셋 완화'는 상한 명시로 대체). 그래도 타임아웃 방지가 항상 우선 — 상한 내 확보된 것만으로 best-effort 완성하라(무한 검색 금지 불변).]
" ;;
...
" ;;
  ...
  if [ -n "${link// }" ]; then
    ...
    if [ "$lkind" = "media" ]; then
      ...
      if [ -s "$trfile" ]; then
        LINK_BLOCK="[🎧 운영자가 준 미디어 링크(${link})를 전사한 전문 — **이 전사문이 곧 원문이다**. 다른 기사를 WebSearch 로 찾지 말고 이 전사 내용만으로 위 출력 포맷대로 큐레이션하라(전사에 없는 사실·수치·인용은 지어내지 마라 · 타임코드 [mm:ss]는 위치 표시일 뿐이니 본문에 옮기지 마라 · frontmatter url = 이 미디어 링크 · media/reporter/date 는 전사 메타에 있는 만큼만). image_sources 는 위 1)의 규칙대로 비워 둔다(사진 = 요약 착지 뒤 이미지 레인):
$(head -c 60000 "$trfile")
]
"
      else
...
"
 ...
 - 이 추출문을 **사실의 축**으로 삼고, 위 1) 보강 우선순위대로 WebSearch(총 3회 상한 내)로 빠진 축을 채워 교차확인한 다이제스트를 내라 — 추출문에서 나온 고유명사·기관·쟁점이 곧 검색어다(제목만으로는 검색어가 안 나오던 글이 여기서 검색 가능해진다).
...
"
 ...
 - 그림에서 사건·인물·기관·제품 같은 고유명사나 쟁점을 뽑았으면 **그 키워드로 WebSearch** 해 관련 보도로 보강하라(상한은 위 3회 규칙 그대로). 제목만으로는 검색어가 안 나오던 글도 그림을 읽으면 검색어가 나온다.
...
[★ 요약 요청 모드 — 운영자가 자연어 + 캡처로 큐레이션을 직접 요청했다.
 1) ⭐ **기본값 = 보강 모드(운영자 260803 「보강이 기본값」 — 구 260704 '전문만으로 바로 큐레이션' 폐지·역전)**: 요청문에 기사 본문급 전문(수백 자 이상)이 들어 있으면 그 전문은 '사실의 축'(원문)이다 — 축만 되풀이해 요약하고 끝내지 말고, WebSearch(아래 상한 내)로 **원문에 빠진 축을 찾아 채워 교차확인한 보강 다이제스트**를 내라. 보강 우선순위 = ① 덩어리 숫자의 내역 분해(총액→구성) ② 반대편·상대 당사자의 주장(원문이 한쪽 입장만 담았으면 필수) ③ 사건 경위·이전 보도 맥락(타임라인 재료) ④ 핵심 실체의 정체(등장 회사·기관·인물이 어떤 곳/누구인지) ⑤ 예정된 다음 단계. ⚠️⚠️ 관점 축 4계약(정본 = 위 문서 입력 처리 0 의 ①~④ 그대로 · analyze 경로와 한 계약 — 초점 보존 · 보강 종속 · 사건 동일성 = 전문의 인물+시점+장소 일치 시만 채택 · 아래 상한은 천장이지 할당량이 아니다). 원문과 보강 사실이 충돌하면 다수·최신 보도를 우선하되 단정 대신 병기하고, 결과가 아직 안 나온 사안은 「미확정」으로 정직하게 표기하라(없는 사실 날조 금지 불변). ⚠️ 유일한 예외 = 아래 프리셋에 「원본 한정」이 있으면 이 보강 검색 전부를 생략한다(그 블록이 항상 우선). ⚠️ **관련이미지 소스(image_sources)는 채우지 않는다**(운영자 260925 「요약이 완료된 이후에 이미지를 찾게」): frontmatter \`image_sources\`는 빈 문자열 그대로 두고 **이미지 소스 수집 목적의 WebSearch 를 돌리지 마라** — 요약이 착지한 뒤 이미지 레인이 네가 쓴 image_query·image_query_en 으로 구글 뉴스를 검색해 사진을 채운다(= image_query 를 이 사건 고유명사로 정확히 쓰는 것이 곧 사진 품질 · 검색 예산은 전부 요약 보강에). 원문 \`url:\` 은 아래 4) 그대로. 전문 없이 본문에 URL만 있으면 그 기사를(운영자가 직접 고른 URL은 오래됐어도 존중), URL·전문 없이 토픽/캡처만 있으면 WebSearch 로 '제일 메이저' 기사 1건(여럿이면 합쳐서 핵심)을 찾는다. ⚠️ **토픽/캡처 검색 시 = 최신 우선(18시간 내)**: 같은 사안이면 **최근 18시간 내 보도 중 가장 메이저한 것**을 골라라(며칠·몇 주 지난 옛 기사가 뉴스요약 피드 상단 채우는 문제 방지 — 운영자 260702). 18시간 내 보도가 없으면 그중 가장 최근 것으로(억지 최신화·날짜 조작 금지), *최신 보도가 있는데 옛 기사를 고르지는 마라*. ⚠️ **검색 상한 = 총 3회(보강 검색 포함 · 260825 운영자 하향 — 구 6회)** — 상한 안에서 안 나오는 축은 비워두고 있는 정보로 best-effort 완성하라(무한 검색으로 타임아웃 나면 아예 요약이 0이 된다 — 요약 완성이 보강보다 항상 우선).
