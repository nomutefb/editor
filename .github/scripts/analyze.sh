...
[⛔ image_sources 오버라이드 — 이 실행(analyze 경로) 한정] frontmatter image_sources 는 **네가 채우지 않는다** — 요약이 착지한 *뒤* 이미지 레인이 네가 쓴 image_query·image_query_en 으로 구글 뉴스를 검색해 채운다(그러니 image_query 를 이 사건 고유명사로 정확히 쓰는 것이 곧 사진 품질이다). **이미지 소스 수집 목적의 WebSearch 를 일절 돌리지 말고** image_sources: \"\" 빈 문자열 그대로 둬라(구 '검색어 바꿔가며 7~10개' 다수 검색 = 분석 지연·타임아웃 주범이라 분리 — \"요약 완성이 관련이미지보다 우선\"의 완결). 사실 확보·교차확인·원문 url 확보용 WebSearch/WebFetch 는 종전 규칙 그대로.
...
' 2>/dev/null || true)"
  ...
  # 랩퍼 코드펜스 벗기기(260817 실사고 봉합 · 운영자 「뉴스 2번 요약이 깨진다」) — 모델이 카드 전체를
  #   ```markdown 으로 감싸고 그 뒤에 자기 보고문("완료 여부"·"3줄 결론")까지 덧붙인 회차의 정규화.
  #   위 sed 는 첫 '---' 부터 취하므로 랩퍼 앞의 여분 구분선부터 통째로 살아남고, 아래 awk 는 '---'·빈 줄만
  #   걷지 펜스는 모른다 → 진짜 frontmatter 가 코드블록 안으로 밀려 뷰어가 그걸 본문으로 그린다
  #   (실측 = 260817-1047 신고 건 · 260812-1906 도 같은 형태 = 모델 변덕이 올 때마다 재현되던 간헐 사고).
  #   랩퍼가 없으면 바이트 무변경(큐 683건 전건 무변형 실측) = 악화 경로 0.
...
PY
  ...
  # ── 전문 붙여넣기 원문 주소 도장(260817 · 운영자 「요약끝난곳에도 좌측상단에 바로가기 버튼 … 떼어진듯」) ──
  #   뷰어 요약 모달 좌상단 원문 버튼(#src)은 frontmatter url 이 있을 때만 활성이다(index.html 4978~4979행) —
  #   버튼이 사라진 게 아니라 갈 곳이 없어 꺼진 것. paste 경로는 art_url="" 라 그 값을 **모델이 검색으로
  #   찾아야만** 채워졌고(프롬프트 = 「몇 번에 안 나오면 빈 문자열」), 그래서 같은 주소가 본문에 실려 온
  #   두 건에서 결과가 갈렸다(실측 260817 = 1001 채움 / 0954 빈값).
  #   그 주소는 이미 폰이 본문 꼬리에 붙여 보내므로 스크립트가 결정론으로 박는다(대기열 행 alt1 과 같은 술어).
  #   가드 = ① paste 경로만 ② url 이 비었을 때만(모델이 원매체를 찾았으면 무접촉) ③ 주소 없으면 무주입 = 종전 동작.
  if [[ "$url" == paste:* ]]; then
    _ufix="$(printf '%s\n' "$out" | python3 .github/scripts/restore_paste_url.py "$f" 2>/tmp/_pu_note || true)"
    if [ -n "${_ufix// }" ] && [ "$_ufix" != "$out" ]; then
      out="$_ufix"
      echo "  원문 주소 주입 — $(tr '\n' '·' < /tmp/_pu_note)"   # 가시성(Actions 로그)
    elif [ -s /tmp/_pu_note ]; then
      echo "  원문 주소 주입 — $(tr '\n' '·' < /tmp/_pu_note)"   # 계측(무변경 사유도 로그)
    fi
  fi
